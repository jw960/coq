...
| SexprStr {v=str} ->
  ...
  ScopeRule (Pcoq.GExtend.s_token (Tok.PIDENT (Some str)), (fun _ -> v_unit))
| tok ->
...
| [] -> KRule (Pcoq.GExtend.r_stop, fun k loc -> k loc [])
| TacNonTerm (na, ScopeRule (scope, inj)) :: tok ->
  let KRule (rule, act) = get_rule tok in
  let rule = Pcoq.GExtend.r_next rule scope in
  let act k e = act (fun loc acc -> k loc ((na, inj e) :: acc)) in
  KRule (rule, act)
| TacTerm t :: tok ->
  let KRule (rule, act) = get_rule tok in
  let rule = Pcoq.GExtend.r_next rule (Pcoq.GExtend.s_token (CLexer.terminal t)) in
...
let perform_notation syn st =
  ...
  in
  let rule = Pcoq.GExtend.production (rule, act mk) in
  let lev = match syn.synext_lev with
  | None -> None
  | Some lev -> Some (string_of_int lev)
  in
  let rule = (lev, None, [rule]) in
  ([Pcoq.ExtendRule (Pltac.tac2expr, None, {Pcoq.pos=None; data=[rule]})], st)
