...
let hashcons (sh_sort,sh_ci,sh_construct,sh_ind,sh_con,sh_na,sh_id) =
  let rec hash_term t =
    match t with
      | Var i ->
        (Var (sh_id i), combinesmall Hashval._1 (Id.hash i))
      | Sort s ->
        (Sort (sh_sort s), combinesmall Hashval._2 (Sorts.hash s))
      | Cast (c, k, t) ->
	let c, hc = sh_rec c in
	let t, ht = sh_rec t in
        (Cast (c, k, t), combinesmall Hashval._3 (combine3 hc (Hashval.of_int (hash_cast_kind k)) ht))
      | Prod (na,t,c) ->
	let t, ht = sh_rec t
	and c, hc = sh_rec c in
        (Prod (sh_na na, t, c), combinesmall Hashval._4 (combine3 (hash_annot Name.hash na) ht hc))
      | Lambda (na,t,c) ->
	let t, ht = sh_rec t
	and c, hc = sh_rec c in
        (Lambda (sh_na na, t, c), combinesmall Hashval._5 (combine3 (hash_annot Name.hash na) ht hc))
      | LetIn (na,b,t,c) ->
	let b, hb = sh_rec b in
	let t, ht = sh_rec t in
	let c, hc = sh_rec c in
        (LetIn (sh_na na, b, t, c), combinesmall Hashval._6 (combine4 (hash_annot Name.hash na) hb ht hc))
      | App (c,l) ->
	let c, hc = sh_rec c in
	let l, hl = hash_term_array l in
        (App (c,l), combinesmall Hashval._7 (combine hl hc))
      | Evar (e,l) ->
	let l, hl = hash_term_array l in
        (Evar (e,l), combinesmall Hashval._8 (combine (Evar.hash e) hl))
      | Const (c,u) ->
	let c' = sh_con c in
	let u', hu = sh_instance u in
        (Const (c', u'), combinesmall Hashval._9 (combine (Constant.SyntacticOrd.hash c) (Hashval.of_int hu)))
      | Ind (ind,u) ->
	let u', hu = sh_instance u in
	(Ind (sh_ind ind, u'), 
         combinesmall Hashval._10 (combine (ind_syntactic_hash ind) (Hashval.of_int hu)))
      | Construct (c,u) ->
	let u', hu = sh_instance u in
	(Construct (sh_construct c, u'),
         combinesmall Hashval._11 (combine (constructor_syntactic_hash c) (Hashval.of_int hu)))
      | Case (ci,p,c,bl) ->
	let p, hp = sh_rec p
	and c, hc = sh_rec c in
	let bl,hbl = hash_term_array bl in
        let hbl = combine (combine hc hp) hbl in
        (Case (sh_ci ci, p, c, bl), combinesmall Hashval._12 hbl)
      | Fix (ln,(lna,tl,bl)) ->
        let bl,hbl = hash_term_array bl in
	let tl,htl = hash_term_array tl in
        ...
        (Fix (ln,(lna,tl,bl)), combinesmall Hashval._13 h)
      | CoFix(ln,(lna,tl,bl)) ->
        let bl,hbl = hash_term_array bl in
	let tl,htl = hash_term_array tl in
        let () = Array.iteri (fun i x -> Array.unsafe_set lna i (sh_na x)) lna in
        ...
        let hna = Array.fold_left fold Hashval._0 lna in
        let h = combine3 hna hbl htl in
        (CoFix (ln,(lna,tl,bl)), combinesmall Hashval._14 h)
      | Meta n ->
        (t, combinesmall Hashval._15 (Hashval.of_int n))
      | Rel n ->
        (t, combinesmall Hashval._16 (Hashval.of_int n))
      | Proj (p,c) ->
        let c, hc = sh_rec c in
        let p' = Projection.hcons p in
          (Proj (p', c), combinesmall Hashval._17 (combine (Projection.SyntacticOrd.hash p') hc))
      | Int i ->
        let (h,l) = Uint63.to_int2 i in
        (t, combinesmall Hashval._18 (combine Hashval.(of_int h) Hashval.(of_int l)))
...
let rec hash t =
  match kind t with
    | Var i -> combinesmall Hashval._1 (Id.hash i)
    | Sort s -> combinesmall Hashval._2 (Sorts.hash s)
    | Cast (c, k, t) ->
      let hc = hash c in
      let ht = hash t in
      combinesmall Hashval._3 (combine3 hc (Hashval.of_int (hash_cast_kind k)) ht)
    | Prod (_, t, c) -> combinesmall Hashval._4 (combine (hash t) (hash c))
    | Lambda (_, t, c) -> combinesmall Hashval._5 (combine (hash t) (hash c))
    | LetIn (_, b, t, c) ->
      combinesmall Hashval._6 (combine3 (hash b) (hash t) (hash c))
    | App (Cast(c, _, _),l) -> hash (mkApp (c,l))
    | App (c,l) ->
      combinesmall Hashval._7 (combine (hash_term_array l) (hash c))
    | Evar (e,l) ->
      combinesmall Hashval._8 (combine (Evar.hash e) (hash_term_array l))
    | Const (c,u) ->
      combinesmall Hashval._9 (combine (Constant.hash c) (Instance.hash u))
    | Ind (ind,u) ->
      combinesmall Hashval._10 (combine (ind_hash ind) (Instance.hash u))
    | Construct (c,u) ->
      combinesmall Hashval._11 (combine (constructor_hash c) (Instance.hash u))
    | Case (_ , p, c, bl) ->
      combinesmall Hashval._12 (combine3 (hash c) (hash p) (hash_term_array bl))
    | Fix (_ln ,(_, tl, bl)) ->
      combinesmall Hashval._13 (combine (hash_term_array bl) (hash_term_array tl))
    | CoFix(_ln, (_, tl, bl)) ->
       combinesmall Hashval._14 (combine (hash_term_array bl) (hash_term_array tl))
    | Meta n -> combinesmall Hashval._15 Hashval.(of_int n)
    | Rel n -> combinesmall Hashval._16 Hashval.(of_int n)
    | Proj (p,c) ->
      combinesmall Hashval._17 (combine (Projection.hash p) (hash c))
    | Int i -> combinesmall Hashval._18 Hashval.(of_int (Uint63.hash i))
...
struct
  ...
  let hash ci =
    ...
    let h3 = Array.fold_left (fun x y -> combine x Hashval.(of_int y)) Hashval._0 ci.ci_cstr_ndecls in
    let h4 = Array.fold_left (fun x y -> combine x Hashval.(of_int y)) Hashval._0 ci.ci_cstr_nargs in
