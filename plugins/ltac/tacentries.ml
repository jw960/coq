...
let atactic n =
  if n = 5 then Pcoq.GExtend.s_nterm Pltac.binder_tactic
  else Pcoq.GExtend.s_nterml Pltac.tactic_expr (string_of_int n)
...
let get_tacentry n m =
  ...
  in
  if check_lvl n then EntryName (rawwit Tacarg.wit_tactic, Pcoq.GExtend.s_self)
  else if check_lvl (n + 1) then EntryName (rawwit Tacarg.wit_tactic, Pcoq.GExtend.s_next)
...
| Extend.Ulist1 s ->
  ...
  EntryName (Rawwit (ListArg typ), Pcoq.GExtend.s_list1 e)
| Extend.Ulist0 s ->
  let EntryName (Rawwit typ, e) = prod_item_of_symbol lev s in
  EntryName (Rawwit (ListArg typ), Pcoq.GExtend.s_list0 e)
| Extend.Ulist1sep (s, sep) ->
  let EntryName (Rawwit typ, e) = prod_item_of_symbol lev s in
  EntryName (Rawwit (ListArg typ), Pcoq.GExtend.s_list1sep e (Pcoq.GExtend.s_token (CLexer.terminal sep)))
| Extend.Ulist0sep (s, sep) ->
  let EntryName (Rawwit typ, e) = prod_item_of_symbol lev s in
  EntryName (Rawwit (ListArg typ), Pcoq.GExtend.s_list0sep e (Pcoq.GExtend.s_token (CLexer.terminal sep)))
| Extend.Uopt s ->
  let EntryName (Rawwit typ, e) = prod_item_of_symbol lev s in
  EntryName (Rawwit (OptArg typ), Pcoq.GExtend.s_opt e)
| Extend.Uentry arg ->
  let ArgT.Any tag = arg in
  let wit = ExtraArg tag in
  EntryName (Rawwit wit, Pcoq.GExtend.s_nterm (genarg_grammar wit))
| Extend.Uentryl (s, n) ->
...
let add_tactic_entry (kn, ml, tg) state =
  ...
  let r = ExtendRule (entry, None, { pos; data=[(None, None, [rules])]}) in
  ([r], state)
...
let create_ltac_quotation name cast (e, l) =
  ...
  | None -> Pcoq.GExtend.s_nterm e
  | Some l -> Pcoq.GExtend.s_nterml e (string_of_int l)
  in
(*   let level = Some "1" in *)
  let level = None in
  let assoc = None in
  ...
  Pcoq.grammar_extend Pltac.tactic_arg None {pos=None; data=[gram]}
...
let argument_extend (type a b c) ~name (arg : (a, b, c) tactic_argument) =
  ...
  | Vernacextend.Arg_rules rules ->
    ...
    let () = Pcoq.grammar_extend e None {pos=None; data=[(None, None, rules)]} in
    e
  in
