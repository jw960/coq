...
let extraction_language x = Lib.add_leaf (None, extr_lang x)
...
let extraction_inline b l =
  ...
  Lib.add_leaf (None, inline_extraction (b,refs))
...
let reset_extraction_inline () = Lib.add_leaf (None, reset_inline ())
...
let extraction_implicit r l =
  ...
  Lib.add_leaf (None, implicit_extraction (Smartlocate.global_with_alias r,l))
...
let extraction_blacklist l =
  let l = List.rev l in
  Lib.add_leaf (None, blacklist_extraction l)
...
let reset_extraction_blacklist () = Lib.add_leaf (None, reset_blacklist ())
...
let extract_constant_inline inline r ids s =
  ...
  match g with
    | GlobRef.ConstRef kn ->
        ...
        Lib.add_leaf (None, inline_extraction (inline,[g]));
        Lib.add_leaf (None, in_customs (g,ids,s))
...
let extract_inductive r s l optstr =
  ...
  match g with
    | GlobRef.IndRef ((kn,i) as ip) ->
        ...
        Lib.add_leaf (None, inline_extraction (true,[g]));
        Lib.add_leaf (None, in_customs (g,[],s));
        Option.iter (fun s -> Lib.add_leaf (None, in_custom_matchs (g,s)))
          optstr;
        List.iteri
          (fun j s ->
             let g = GlobRef.ConstructRef (ip,succ j) in
             Lib.add_leaf (None, inline_extraction (true,[g]));
             Lib.add_leaf (None, in_customs (g,[],s))) l
