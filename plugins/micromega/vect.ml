...
let hash v =
  let rec hash i = function
    | [] ->
      i
    | (vr, vl) :: l ->
      hash (i + Hashtbl.hash (vr, float_of_num vl)) l
  in
...
let pp_var_num pp_var o (v, n) =
  if Int.equal v 0 then
    ...
    | Int 1 ->
      pp_var o v
    | Int -1 ->
      Printf.fprintf o "-%a" pp_var v
    | Int 0 ->
      ()
    | _ ->
      Printf.fprintf o "%s*%a" (string_of_num n) pp_var v
...
let rec pp_gen pp_var o v =
  ...
  | [] ->
    output_string o "0"
  | [e] ->
    pp_var_num pp_var o e
  | e :: l ->
    Printf.fprintf o "%a + %a" (pp_var_num pp_var) e (pp_gen pp_var) l
...
let mul z t =
  ...
  | Int 1 ->
    t
  | _ ->
    List.map (fun (i, n) -> (i, mult_num z n)) t
...
let rec tail (v : var) (vect : t) =
  ...
  | (v', vl) :: vect' -> (
    ...
    | 0 ->
      Some (vl, vect) (* Ok, found *)
    | -1 ->
      tail v vect' (* Might be in the tail *)
    | _ ->
      None )
