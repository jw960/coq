...
let rec nth n0 l default =
  ...
  | S m -> (
    match l with [] -> default | _ :: t0 -> nth m t0 default )

(** val rev_append : 'a1 list -> 'a1 list -> 'a1 list **)

let rec rev_append l l' =
  match l with [] -> l' | a :: l0 -> rev_append l0 (a :: l')

(** val map : ('a1 -> 'a2) -> 'a1 list -> 'a2 list **)

let rec map f = function [] -> [] | a :: t0 -> f a :: map f t0

(** val fold_left : ('a1 -> 'a2 -> 'a1) -> 'a2 list -> 'a1 -> 'a1 **)

let rec fold_left f l a0 =
  match l with [] -> a0 | b :: t0 -> fold_left f t0 (f a0 b)
...
module Coq_Pos = struct
  ...
  let rec add x y =
    ...
    | XO p -> (
      ...
      | XH ->
        XI p )
    | XH -> (
      match y with XI q0 -> XO (succ q0) | XO q0 -> XI q0 | XH -> XO XH )
  ...
  and add_carry x y =
    ...
    | XO p -> (
      ...
      | XH ->
        XO (succ p) )
    | XH -> (
      match y with XI q0 -> XI (succ q0) | XO q0 -> XO (succ q0) | XH -> XI XH )
  ...
  let double_mask = function IsPos p -> IsPos (XO p) | x0 -> x0
  ...
  let sub x y = match sub_mask x y with IsPos z0 -> z0 | _ -> XH
  ...
  let gcd a b = gcdn (Coq__1.add (size_nat a) (size_nat b)) a b
...
module N = struct
  ...
  let of_nat = function O -> N0 | S n' -> Npos (Coq_Pos.of_succ_nat n')
end
...
module Z = struct
  ...
  let rec pos_sub x y =
    ...
    | XI p -> (
      ...
      | XI q0 ->
        double (pos_sub p q0)
      | XO q0 ->
        succ_double (pos_sub p q0)
      | XH ->
        Zpos (XO p) )
    | XO p -> (
      match y with
      | XI q0 ->
        pred_double (pos_sub p q0)
      | XO q0 ->
        double (pos_sub p q0)
      | XH ->
        Zpos (Coq_Pos.pred_double p) )
  ...
  let mul x y =
    ...
    | Zpos x' -> (
      ...
      | Z0 ->
        Z0
      | Zpos y' ->
        Zpos (Coq_Pos.mul x' y')
      | Zneg y' ->
        Zneg (Coq_Pos.mul x' y') )
    | Zneg x' -> (
      match y with
      | Z0 ->
        Z0
  ...
  let pow_pos z0 = Coq_Pos.iter (mul z0) (Zpos XH)

  (** val pow : z -> z -> z **)

  let pow x = function Z0 -> Zpos XH | Zpos p -> pow_pos x p | Zneg _ -> Z0
  ...
  let compare x y =
    match x with
    | Z0 -> (
      match y with Z0 -> Eq | Zpos _ -> Lt | Zneg _ -> Gt )
    | Zpos x' -> (
      match y with Zpos y' -> Coq_Pos.compare x' y' | _ -> Gt )
    | Zneg x' -> (
      match y with Zneg y' -> compOpp (Coq_Pos.compare x' y') | _ -> Lt )
  ...
  let max n0 m = match compare n0 m with Lt -> m | _ -> n0

  (** val abs : z -> z **)

  let abs = function Zneg p -> Zpos p | x -> x

  (** val to_N : z -> n **)

  let to_N = function Zpos p -> Npos p | _ -> N0

  (** val of_nat : nat -> z **)

  let of_nat = function O -> Z0 | S n1 -> Zpos (Coq_Pos.of_succ_nat n1)

  (** val of_N : n -> z **)

  let of_N = function N0 -> Z0 | Npos p -> Zpos p
  ...
  let rec pos_div_eucl a b =
    ...
    | XI a' ->
      ...
      if ltb r' b then (mul (Zpos (XO XH)) q0, r')
    ...
    | XH ->
      if leb (Zpos (XO XH)) b then (Z0, Zpos XH) else (Zpos XH, Z0)
...
let zeq_bool x y = match Z.compare x y with Eq -> true | _ -> false
...
let rec peq ceqb p p' =
  match p with
  | Pc c -> (
    match p' with Pc c' -> ceqb c c' | _ -> false )
  | Pinj (j, q0) -> (
    match p' with
    | Pinj (j', q') -> (
      match Coq_Pos.compare j j' with Eq -> peq ceqb q0 q' | _ -> false )
    | _ ->
      false )
  | PX (p2, i, q0) -> (
    match p' with
    | PX (p'0, i', q') -> (
      ...
      | Eq ->
        if peq ceqb p2 p'0 then peq ceqb q0 q' else false
      | _ ->
        false )
    | _ ->
      false )
...
let mkPX cO ceqb p i q0 =
  match p with
  | Pc c ->
    if ceqb c cO then mkPinj XH q0 else PX (p, i, q0)
  | Pinj (_, _) ->
    PX (p, i, q0)
  | PX (p', i', q') ->
    if peq ceqb q' (p0 cO) then PX (p', Coq_Pos.add i' i, q0) else PX (p, i, q0)

(** val mkXi : 'a1 -> 'a1 -> positive -> 'a1 pol **)

let mkXi cO cI i = PX (p1 cI, i, p0 cO)

(** val mkX : 'a1 -> 'a1 -> 'a1 pol **)

let mkX cO cI = mkXi cO cI XH
...
let rec popp copp = function
  ...
  | Pinj (j, q0) ->
    Pinj (j, popp copp q0)
  | PX (p2, i, q0) ->
    PX (popp copp p2, i, popp copp q0)
...
let rec paddC cadd p c =
  ...
  | Pc c1 ->
    Pc (cadd c1 c)
  | Pinj (j, q0) ->
    Pinj (j, paddC cadd q0 c)
  | PX (p2, i, q0) ->
    PX (p2, i, paddC cadd q0 c)
...
let rec psubC csub p c =
  ...
  | Pc c1 ->
    Pc (csub c1 c)
  | Pinj (j, q0) ->
    Pinj (j, psubC csub q0 c)
  | PX (p2, i, q0) ->
    PX (p2, i, psubC csub q0 c)
...
let rec paddI cadd pop q0 j = function
  | Pc c ->
    mkPinj j (paddC cadd q0 c)
  | Pinj (j', q') -> (
    match Z.pos_sub j' j with
    | Z0 ->
      mkPinj j (pop q' q0)
    | Zpos k ->
      mkPinj j (pop (Pinj (k, q')) q0)
    | Zneg k ->
      mkPinj j' (paddI cadd pop q0 k q') )
  | PX (p2, i, q') -> (
    match j with
    | XI j0 ->
      PX (p2, i, paddI cadd pop q0 (XO j0) q')
    | XO j0 ->
      PX (p2, i, paddI cadd pop q0 (Coq_Pos.pred_double j0) q')
    | XH ->
      PX (p2, i, pop q' q0) )
...
let rec psubI cadd copp pop q0 j = function
  | Pc c ->
    mkPinj j (paddC cadd (popp copp q0) c)
  | Pinj (j', q') -> (
    match Z.pos_sub j' j with
    | Z0 ->
      mkPinj j (pop q' q0)
    | Zpos k ->
      mkPinj j (pop (Pinj (k, q')) q0)
    | Zneg k ->
      mkPinj j' (psubI cadd copp pop q0 k q') )
  | PX (p2, i, q') -> (
    match j with
    | XI j0 ->
      PX (p2, i, psubI cadd copp pop q0 (XO j0) q')
    | XO j0 ->
...
let rec paddX cO ceqb pop p' i' p =
  match p with
  | Pc _ ->
    PX (p', i', p)
  | Pinj (j, q') -> (
    ...
    | XI j0 ->
      PX (p', i', Pinj (XO j0, q'))
    | XO j0 ->
      PX (p', i', Pinj (Coq_Pos.pred_double j0, q'))
    | XH ->
      PX (p', i', q') )
  | PX (p2, i, q') -> (
    match Z.pos_sub i i' with
    | Z0 ->
      mkPX cO ceqb (pop p2 p') i q'
    | Zpos k ->
      mkPX cO ceqb (pop (PX (p2, k, p0 cO)) p') i' q'
    | Zneg k ->
      mkPX cO ceqb (paddX cO ceqb pop p' k p2) i q' )

(** val psubX :
    'a1 -> ('a1 -> 'a1) -> ('a1 -> 'a1 -> bool) -> ('a1 pol -> 'a1 pol -> 'a1
    pol) -> 'a1 pol -> positive -> 'a1 pol -> 'a1 pol **)

let rec psubX cO copp ceqb pop p' i' p =
  match p with
  | Pc _ ->
    PX (popp copp p', i', p)
  | Pinj (j, q') -> (
  ...
  | PX (p2, i, q') -> (
    ...
    | Zpos k ->
      mkPX cO ceqb (pop (PX (p2, k, p0 cO)) p') i' q'
    | Zneg k ->
      mkPX cO ceqb (psubX cO copp ceqb pop p' k p2) i q' )
...
let rec padd cO cadd ceqb p = function
  | Pc c' ->
    paddC cadd p c'
  | Pinj (j', q') ->
    paddI cadd (padd cO cadd ceqb) q' j' p
  | PX (p'0, i', q') -> (
    match p with
    | Pc c ->
      PX (p'0, i', paddC cadd q' c)
    | Pinj (j, q0) -> (
      match j with
      | XI j0 ->
        PX (p'0, i', padd cO cadd ceqb (Pinj (XO j0, q0)) q')
      | XO j0 ->
        PX (p'0, i', padd cO cadd ceqb (Pinj (Coq_Pos.pred_double j0, q0)) q')
      | XH ->
        PX (p'0, i', padd cO cadd ceqb q0 q') )
    | PX (p2, i, q0) -> (
      match Z.pos_sub i i' with
      | Z0 ->
      ...
      | Zpos k ->
        mkPX cO ceqb
          (padd cO cadd ceqb (PX (p2, k, p0 cO)) p'0)
          i' (padd cO cadd ceqb q0 q')
      | Zneg k ->
        mkPX cO ceqb
          (paddX cO ceqb (padd cO cadd ceqb) p'0 k p2)
          i (padd cO cadd ceqb q0 q') ) )
...
let rec psub cO cadd csub copp ceqb p = function
  | Pc c' ->
    psubC csub p c'
  | Pinj (j', q') ->
    psubI cadd copp (psub cO cadd csub copp ceqb) q' j' p
  | PX (p'0, i', q') -> (
    match p with
    | Pc c ->
      PX (popp copp p'0, i', paddC cadd (popp copp q') c)
    | Pinj (j, q0) -> (
      match j with
      | XI j0 ->
        PX (popp copp p'0, i', psub cO cadd csub copp ceqb (Pinj (XO j0, q0)) q')
      | XO j0 ->
        PX
          ( popp copp p'0
          , i'
          , psub cO cadd csub copp ceqb (Pinj (Coq_Pos.pred_double j0, q0)) q'
          )
      | XH ->
    ...
    | PX (p2, i, q0) -> (
      ...
      | Z0 ->
        mkPX cO ceqb
          (psub cO cadd csub copp ceqb p2 p'0)
          i
      ...
      | Zneg k ->
        mkPX cO ceqb
          (psubX cO copp ceqb (psub cO cadd csub copp ceqb) p'0 k p2)
          i
          (psub cO cadd csub copp ceqb q0 q') ) )
...
let rec pmulC_aux cO cmul ceqb p c =
  ...
  | Pc c' ->
    Pc (cmul c' c)
  | Pinj (j, q0) ->
    mkPinj j (pmulC_aux cO cmul ceqb q0 c)
  | PX (p2, i, q0) ->
...
let pmulC cO cI cmul ceqb p c =
  if ceqb c cO then p0 cO
  else if ceqb c cI then p
  else pmulC_aux cO cmul ceqb p c
...
let rec pmulI cO cI cmul ceqb pmul0 q0 j = function
  | Pc c ->
    mkPinj j (pmulC cO cI cmul ceqb q0 c)
  | Pinj (j', q') -> (
    match Z.pos_sub j' j with
    | Z0 ->
      mkPinj j (pmul0 q' q0)
    | Zpos k ->
      mkPinj j (pmul0 (Pinj (k, q')) q0)
    | Zneg k ->
      mkPinj j' (pmulI cO cI cmul ceqb pmul0 q0 k q') )
  | PX (p', i', q') -> (
    match j with
    | XI j' ->
      mkPX cO ceqb
        (pmulI cO cI cmul ceqb pmul0 q0 j p')
        i'
...
let rec pmul cO cI cadd cmul ceqb p p'' =
  match p'' with
  | Pc c ->
    pmulC cO cI cmul ceqb p c
  | Pinj (j', q') ->
  ...
  | PX (p', i', q') -> (
    ...
    | PX (p2, i, q0) ->
      ...
      padd cO cadd ceqb
        (mkPX cO ceqb
           (padd cO cadd ceqb (mkPX cO ceqb pP' i (p0 cO)) qP')
           i' (p0 cO))
...
let rec psquare cO cI cadd cmul ceqb = function
  | Pc c ->
    Pc (cmul c c)
  | Pinj (j, q0) ->
    Pinj (j, psquare cO cI cadd cmul ceqb q0)
  | PX (p2, i, q0) ->
    let twoPQ =
...
let mk_X cO cI j = mkPinj_pred j (mkX cO cI)
    ...
    subst_l
      (pmul cO cI cadd cmul ceqb
         (ppow_pos cO cI cadd cmul ceqb subst_l
            (ppow_pos cO cI cadd cmul ceqb subst_l res p p3)
            p p3)
         p)
  | XO p3 ->
    ppow_pos cO cI cadd cmul ceqb subst_l
      ...
      p p3
  | XH ->
    subst_l (pmul cO cI cadd cmul ceqb res p)
...
let ppow_N cO cI cadd cmul ceqb subst_l p = function
  | N0 ->
    p1 cI
  | Npos p2 ->
    ppow_pos cO cI cadd cmul ceqb subst_l (p1 cI) p p2
...
let rec norm_aux cO cI cadd cmul csub copp ceqb = function
  ...
  | PEmul (pe1, pe2) ->
    pmul cO cI cadd cmul ceqb
      ...
      (norm_aux cO cI cadd cmul csub copp ceqb pe2)
  | PEopp pe1 ->
    popp copp (norm_aux cO cI cadd cmul csub copp ceqb pe1)
  | PEpow (pe1, n0) ->
    ppow_N cO cI cadd cmul ceqb
      (fun p -> p)
      (norm_aux cO cI cadd cmul csub copp ceqb pe1)
      n0
...
let rec mapX f = function
  | X x ->
    X (f x)
  | Cj (f1, f2) ->
    Cj (mapX f f1, mapX f f2)
  | D (f1, f2) ->
    D (mapX f f1, mapX f f2)
  | N f1 ->
    N (mapX f f1)
  | I (f1, o, f2) ->
    I (mapX f f1, o, mapX f f2)
  | x ->
    x
...
let cons_id id l = match id with Some id0 -> id0 :: l | None -> l

(** val ids_of_formula : ('a1, 'a2, 'a3, 'a4) gFormula -> 'a4 list **)

let rec ids_of_formula = function
  | I (_, id, f') ->
    cons_id id (ids_of_formula f')
  | _ ->
    []
...
let rec collect_annot = function
  | A (_, a) ->
    [a]
  | Cj (f1, f2) ->
    app (collect_annot f1) (collect_annot f2)
  | D (f1, f2) ->
    app (collect_annot f1) (collect_annot f2)
  | N f0 ->
    collect_annot f0
...
let rec map_bformula fct = function
  ...
  | A (a, t0) ->
    A (fct a, t0)
  | Cj (f1, f2) ->
    Cj (map_bformula fct f1, map_bformula fct f2)
  | D (f1, f2) ->
    D (map_bformula fct f1, map_bformula fct f2)
  | N f0 ->
    N (map_bformula fct f0)
  | I (f1, a, f2) ->
    I (map_bformula fct f1, a, map_bformula fct f2)
...
let rec add_term unsat deduce t0 = function
  | [] -> (
    ...
    | Some u ->
      if unsat u then None else Some [t0]
    | None ->
      Some [t0] )
  | t' :: cl0 -> (
    match deduce (fst t0) (fst t') with
    | Some u -> (
      if unsat u then None
      else
...
let or_clause_cnf unsat deduce t0 f =
  match t0 with [] -> f | _ :: _ -> xor_clause_cnf unsat deduce t0 f
...
let and_cnf_opt f1 f2 =
  if if is_cnf_ff f1 then true else is_cnf_ff f2 then cnf_ff else and_cnf f1 f2
...
let or_cnf_opt unsat deduce f1 f2 =
  if if is_cnf_tt f1 then true else is_cnf_tt f2 then cnf_tt
...
let rec xcnf unsat deduce normalise1 negate0 pol0 = function
  | TT ->
    if pol0 then cnf_tt else cnf_ff
  | FF ->
    if pol0 then cnf_ff else cnf_tt
  | X _ ->
    cnf_ff
  | A (x, t0) ->
    if pol0 then normalise1 x t0 else negate0 x t0
  | Cj (e1, e2) ->
    if pol0 then
      and_cnf_opt
        (xcnf unsat deduce normalise1 negate0 pol0 e1)
  ...
  | D (e1, e2) ->
    if pol0 then
      ...
      and_cnf_opt
        ...
        (xcnf unsat deduce normalise1 negate0 pol0 e2)
  | N e ->
    xcnf unsat deduce normalise1 negate0 (negb pol0) e
  | I (e1, _, e2) ->
    if pol0 then
      or_cnf_opt unsat deduce
...
let rec radd_term unsat deduce t0 = function
  | [] -> (
    match deduce (fst t0) (fst t0) with
    | Some u ->
      if unsat u then Inr [snd t0] else Inl [t0]
    | None ->
      Inl [t0] )
  | t' :: cl0 -> (
    match deduce (fst t0) (fst t') with
    | Some u -> (
      if unsat u then Inr [snd t0; snd t']
      else
...
let ror_clause_cnf unsat deduce t0 f =
  match t0 with [] -> (f, []) | _ :: _ -> xror_clause_cnf unsat deduce t0 f
...
let rec rxcnf unsat deduce normalise1 negate0 polarity = function
  | TT ->
    if polarity then (cnf_tt, []) else (cnf_ff, [])
  | FF ->
    if polarity then (cnf_ff, []) else (cnf_tt, [])
  | X _ ->
    (cnf_ff, [])
  | A (x, t0) ->
  ...
  | Cj (e1, e2) ->
    ...
    if polarity then (and_cnf_opt e3 e4, rev_append t1 t2)
    else
  ...
  | N e ->
    rxcnf unsat deduce normalise1 negate0 (negb polarity) e
  | I (e1, _, e2) ->
    let e3, t1 = rxcnf unsat deduce normalise1 negate0 (negb polarity) e1 in
    if polarity then
      if is_cnf_ff e3 then rxcnf unsat deduce normalise1 negate0 polarity e2
      else
        let e4, t2 = rxcnf unsat deduce normalise1 negate0 polarity e2 in
        let f', t' = ror_cnf_opt unsat deduce e3 e4 in
        (f', rev_append t1 (rev_append t2 t'))
...
let rec aformula to_constr = function
  ...
  | D (f1, f2) ->
    to_constr.mkD (aformula to_constr f1) (aformula to_constr f2)
  | N f0 ->
    to_constr.mkN (aformula to_constr f0)
  | I (f1, _, f2) ->
    to_constr.mkI (aformula to_constr f1) (aformula to_constr f2)

(** val is_X : ('a1, 'a2, 'a3, 'a4) tFormula -> 'a3 option **)

let is_X = function X p -> Some p | _ -> None
...
let mk_arrow o f1 f2 =
  match o with
  | Some _ -> (
    match is_X f1 with Some _ -> f2 | None -> I (f1, o, f2) )
  | None ->
    I (f1, None, f2)
...
let rec abst_form to_constr needA pol0 = function
  | TT ->
    if pol0 then TT else X to_constr.mkTT
  | FF ->
    if pol0 then X to_constr.mkFF else FF
  | X p ->
    X p
  | A (x, t0) ->
    if needA t0 then A (x, t0) else X (to_constr.mkA x t0)
  | Cj (f1, f2) ->
    let f3 = abst_form to_constr needA pol0 f1 in
    let f4 = abst_form to_constr needA pol0 f2 in
    if pol0 then abs_and to_constr f3 f4 (fun x x0 -> Cj (x, x0))
    else abs_or to_constr f3 f4 (fun x x0 -> Cj (x, x0))
  | D (f1, f2) ->
    let f3 = abst_form to_constr needA pol0 f1 in
    let f4 = abst_form to_constr needA pol0 f2 in
    if pol0 then abs_or to_constr f3 f4 (fun x x0 -> D (x, x0))
    else abs_and to_constr f3 f4 (fun x x0 -> D (x, x0))
  | N f0 -> (
    let f1 = abst_form to_constr needA (negb pol0) f0 in
    match is_X f1 with Some a -> X (to_constr.mkN a) | None -> N f1 )
  | I (f1, o, f2) ->
    let f3 = abst_form to_constr needA (negb pol0) f1 in
    let f4 = abst_form to_constr needA pol0 f2 in
    if pol0 then abs_or to_constr f3 f4 (mk_arrow o)
...
let cneqb ceqb x y = negb (ceqb x y)

(** val cltb :
    ('a1 -> 'a1 -> bool) -> ('a1 -> 'a1 -> bool) -> 'a1 -> 'a1 -> bool **)

let cltb ceqb cleb x y = cleb x y && cneqb ceqb x y
...
let opMult o o' =
  ...
  | Equal ->
    Some Equal
  | NonEqual -> (
    match o' with Equal -> Some Equal | NonEqual -> Some NonEqual | _ -> None )
  | Strict -> (
    match o' with NonEqual -> None | _ -> Some o' )
  | NonStrict -> (
    match o' with Equal -> Some Equal | NonEqual -> None | _ -> Some NonStrict )
...
let opAdd o o' =
  ...
  | Equal ->
    Some o'
  | NonEqual -> (
    match o' with Equal -> Some NonEqual | _ -> None )
  | Strict -> (
    match o' with NonEqual -> None | _ -> Some Strict )
  | NonStrict -> (
    match o' with Equal -> Some NonStrict | NonEqual -> None | x -> Some x )
...
let map_option f = function Some x -> f x | None -> None
...
let map_option2 f o o' =
  match o with
  | Some x -> (
    match o' with Some x' -> f x x' | None -> None )
  | None ->
    None
...
let nformula_times_nformula cO cI cplus ctimes ceqb f1 f2 =
  ...
  let e2, o2 = f2 in
  map_option
    (fun x -> Some (pmul cO cI cplus ctimes ceqb e1 e2, x))
    (opMult o1 o2)
...
let rec eval_Psatz cO cI cplus ctimes ceqb cleb l = function
  | PsatzIn n0 ->
    Some (nth n0 l (Pc cO, Equal))
  | PsatzSquare e0 ->
    Some (psquare cO cI cplus ctimes ceqb e0, NonStrict)
  | PsatzMulC (re, e0) ->
    map_option
      (pexpr_times_nformula cO cI cplus ctimes ceqb re)
      (eval_Psatz cO cI cplus ctimes ceqb cleb l e0)
  | PsatzMulE (f1, f2) ->
    map_option2
      (nformula_times_nformula cO cI cplus ctimes ceqb)
      (eval_Psatz cO cI cplus ctimes ceqb cleb l f1)
      (eval_Psatz cO cI cplus ctimes ceqb cleb l f2)
  | PsatzAdd (f1, f2) ->
    map_option2
      (nformula_plus_nformula cO cplus ceqb)
      (eval_Psatz cO cI cplus ctimes ceqb cleb l f1)
      (eval_Psatz cO cI cplus ctimes ceqb cleb l f2)
  | PsatzC c ->
    if cltb ceqb cleb cO c then Some (Pc c, Strict) else None
  | PsatzZ ->
    Some (Pc cO, Equal)
...
let check_normalised_formulas cO cI cplus ctimes ceqb cleb l cm =
  ...
  | Some f ->
    check_inconsistent cO ceqb cleb f
  | None ->
    false
...
let normalise cO cI cplus ctimes cminus copp ceqb f =
  ...
  match op with
  | OpEq ->
    (psub0 cO cplus cminus copp ceqb lhs0 rhs0, Equal)
  | OpNEq ->
    (psub0 cO cplus cminus copp ceqb lhs0 rhs0, NonEqual)
  | OpLe ->
    (psub0 cO cplus cminus copp ceqb rhs0 lhs0, NonStrict)
  | OpGe ->
    (psub0 cO cplus cminus copp ceqb lhs0 rhs0, NonStrict)
  | OpLt ->
    (psub0 cO cplus cminus copp ceqb rhs0 lhs0, Strict)
  | OpGt ->
    (psub0 cO cplus cminus copp ceqb lhs0 rhs0, Strict)
...
let xnormalise copp = function
  | e, o -> (
    match o with
    | Equal ->
      [(e, Strict); (popp0 copp e, Strict)]
    | NonEqual ->
      [(e, Equal)]
    | Strict ->
      [(popp0 copp e, NonStrict)]
    | NonStrict ->
      [(popp0 copp e, Strict)] )
...
let cnf_normalise cO cI cplus ctimes cminus copp ceqb cleb t0 tg =
  ...
  if check_inconsistent cO ceqb cleb f then cnf_ff
...
let cnf_negate cO cI cplus ctimes cminus copp ceqb cleb t0 tg =
  let f = normalise cO cI cplus ctimes cminus copp ceqb t0 in
  if check_inconsistent cO ceqb cleb f then cnf_tt
...
let rec xdenorm jmp = function
  | Pc c ->
    PEc c
  | Pinj (j, p2) ->
    xdenorm (Coq_Pos.add j jmp) p2
  | PX (p2, j, q0) ->
    PEadd
      ( PEmul (xdenorm jmp p2, PEpow (PEX jmp, Npos j))
      , xdenorm (Coq_Pos.succ jmp) q0 )

(** val denorm : 'a1 pol -> 'a1 pExpr **)

let denorm p = xdenorm XH p
...
let rec map_PExpr c_of_S = function
  ...
  | PEX p ->
    PEX p
  | PEadd (e1, e2) ->
    PEadd (map_PExpr c_of_S e1, map_PExpr c_of_S e2)
  | PEsub (e1, e2) ->
    PEsub (map_PExpr c_of_S e1, map_PExpr c_of_S e2)
  | PEmul (e1, e2) ->
    PEmul (map_PExpr c_of_S e1, map_PExpr c_of_S e2)
  | PEopp e0 ->
    PEopp (map_PExpr c_of_S e0)
  | PEpow (e0, n0) ->
    PEpow (map_PExpr c_of_S e0, n0)
...
let simpl_cone cO cI ctimes ceqb e =
  match e with
  | PsatzSquare t0 -> (
    match t0 with
    | Pc c ->
      if ceqb cO c then PsatzZ else PsatzC (ctimes c c)
    | _ ->
      PsatzSquare t0 )
  | PsatzMulE (t1, t2) -> (
    match t1 with
    | PsatzMulE (x, x0) -> (
      ...
      | PsatzC p2 -> (
        match t2 with
        | PsatzC c ->
          PsatzMulE (PsatzC (ctimes c p2), x0)
        | PsatzZ ->
          PsatzZ
        | _ ->
          e )
      | _ -> (
        ...
        | PsatzC p2 -> (
          match t2 with
          | PsatzC c ->
            PsatzMulE (PsatzC (ctimes c p2), x)
          | PsatzZ ->
            PsatzZ
          | _ ->
            e )
  ...
  | PsatzAdd (t1, t2) -> (
    ...
    | PsatzZ ->
      t2
    | _ -> (
      match t2 with PsatzZ -> t1 | _ -> PsatzAdd (t1, t2) ) )
  | _ ->
    e

module PositiveSet = struct
...
let qmult x y = {qnum = Z.mul x.qnum y.qnum; qden = Coq_Pos.mul x.qden y.qden}

(** val qopp : q -> q **)

let qopp x = {qnum = Z.opp x.qnum; qden = x.qden}

(** val qminus : q -> q -> q **)

let qminus x y = qplus x (qopp y)
...
let qinv x =
  ...
  | Z0 ->
    {qnum = Z0; qden = XH}
  | Zpos p ->
    {qnum = Zpos x.qden; qden = p}
  | Zneg p ->
    {qnum = Zneg x.qden; qden = p}

(** val qpower_positive : q -> positive -> q **)

let qpower_positive = pow_pos qmult
...
let rec singleton default x v =
  match x with
  | XI p ->
    Branch (Empty, default, singleton default p v)
  | XO p ->
    Branch (singleton default p v, default, Empty)
  | XH ->
    Elt v
...
let rec vm_add default x v = function
  | Empty ->
    singleton default x v
  | Elt vl -> (
    match x with
    | XI p ->
      Branch (Empty, vl, singleton default p v)
    | XO p ->
      Branch (singleton default p v, vl, Empty)
    | XH ->
      Elt v )
  | Branch (l, o, r) -> (
    match x with
    | XI p ->
      Branch (l, o, vm_add default p v r)
    | XO p ->
      Branch (vm_add default p v l, o, r)
    | XH ->
      Branch (l, v, r) )
...
let rec zeval_const = function
  ...
  | PEopp e0 ->
    map_option (fun x -> Some (Z.opp x)) (zeval_const e0)
  | PEpow (e1, n0) ->
...
let psub1 = psub0 Z0 Z.add Z.sub Z.opp zeq_bool

(** val padd1 : z pol -> z pol -> z pol **)

let padd1 = padd0 Z0 Z.add zeq_bool

(** val normZ : z pExpr -> z pol **)

let normZ = norm Z0 (Zpos XH) Z.add Z.mul Z.sub Z.opp zeq_bool

(** val zunsat : z nFormula -> bool **)

let zunsat = check_inconsistent Z0 zeq_bool Z.leb

(** val zdeduce : z nFormula -> z nFormula -> z nFormula option **)

let zdeduce = nformula_plus_nformula Z0 Z.add zeq_bool
...
let xnnormalise t0 =
  ...
  match o with
  | OpEq ->
    (psub1 rhs0 lhs0, Equal)
  | OpNEq ->
    (psub1 rhs0 lhs0, NonEqual)
  | OpLe ->
    (psub1 rhs0 lhs0, NonStrict)
  | OpGe ->
    (psub1 lhs0 rhs0, NonStrict)
  | OpLt ->
    (psub1 rhs0 lhs0, Strict)
  | OpGt ->
    (psub1 lhs0 rhs0, Strict)
...
let xnormalise0 = function
  | e, o -> (
    ...
    | NonEqual ->
      [(e, Equal)]
    | Strict ->
      [(psub1 (Pc Z0) e, NonStrict)]
    | NonStrict ->
      [(psub1 (Pc (Zneg XH)) e, NonStrict)] )
...
let cnfZ f = rxcnf zunsat zdeduce normalise0 negate true f

(** val ceiling : z -> z -> z **)

let ceiling a b =
  let q0, r = Z.div_eucl a b in
  match r with Z0 -> q0 | _ -> Z.add q0 (Zpos XH)
...
let zgcdM x y = Z.max (Z.gcd x y) (Zpos XH)
...
let rec zdiv_pol p x =
  ...
  | Pc c ->
    Pc (Z.div c x)
  | Pinj (j, p2) ->
    Pinj (j, zdiv_pol p2 x)
  | PX (p2, j, q0) ->
    PX (zdiv_pol p2 x, j, zdiv_pol q0 x)

(** val makeCuttingPlane : z polC -> z polC * z **)

let makeCuttingPlane p =
  let g, c = zgcd_pol p in
  if Z.gtb g Z0 then (zdiv_pol (psubC Z.sub p c) g, Z.opp (ceiling (Z.opp c) g))
  else (p, Z0)
...
let genCuttingPlane = function
  | e, op -> (
    ...
    | Equal ->
      ...
      if Z.gtb g Z0 && negb (zeq_bool c Z0) && negb (zeq_bool (Z.gcd g c) g)
      then None
    ...
    | NonStrict ->
      Some (makeCuttingPlane e, NonStrict) )
...
let is_pol_Z0 = function
  | Pc z0 -> (
    match z0 with Z0 -> true | _ -> false )
  | _ ->
    false

(** val eval_Psatz0 : z nFormula list -> zWitness -> z nFormula option **)

let eval_Psatz0 = eval_Psatz Z0 (Zpos XH) Z.add Z.mul zeq_bool Z.leb

(** val valid_cut_sign : op1 -> bool **)

let valid_cut_sign = function Equal -> true | NonStrict -> true | _ -> false
...
module Vars = struct
  ...
  let rec add i = function
    | Leaf -> (
      match i with
      | XI i0 ->
        Node (Leaf, false, add i0 Leaf)
      | XO i0 ->
        Node (add i0 Leaf, false, Leaf)
      | XH ->
        Node (Leaf, true, Leaf) )
    | Node (l, o, r) -> (
      match i with
  ...
  let rec xfold f m v i =
    ...
    | Leaf ->
      v
    | Node (l, b, r) ->
      if b then xfold f r (f (rev i) (xfold f l v (XO i))) (XI i)
      else xfold f r (xfold f l v (XO i)) (XI i)

  (** val fold : (elt -> 'a1 -> 'a1) -> t -> 'a1 -> 'a1 **)

  let fold f m i = xfold f m i XH
end
...
let bound_var v = {flhs = PEX v; fop = OpGe; frhs = PEc Z0}

(** val mk_eq_pos : positive -> positive -> positive -> z formula **)

let mk_eq_pos x y t0 = {flhs = PEX x; fop = OpEq; frhs = PEsub (PEX y, PEX t0)}
...
let bound_vars tag_of_var fr v =
  Vars.fold
    (fun k acc ->
      ...
      Cj
        ( Cj
            ( A (mk_eq_pos k y z0, tag_of_var fr k None)
            , Cj
                ( A (bound_var y, tag_of_var fr k (Some false))
                , A (bound_var z0, tag_of_var fr k (Some true)) ) )
        , acc ))
    v TT
...
let bound_problem_fr tag_of_var fr f =
  ...
  I (bound_vars tag_of_var fr v, None, f)
...
let rec zChecker l = function
  ...
  | RatProof (w, pf0) -> (
    ...
    | Some f ->
      if zunsat f then true else zChecker (f :: l) pf0
    | None ->
      false )
  | CutProof (w, pf0) -> (
    match eval_Psatz0 l w with
      ...
      | Some f2 -> (
        ...
        | Some p -> (
          ...
          | Some p3 ->
            ...
            let e2, z2 = p4 in
            if
              (valid_cut_sign op3 && valid_cut_sign op4)
...
let zTautoChecker f w =
  tauto_checker zunsat zdeduce normalise0 negate
    (fun cl -> zChecker (map fst cl))
    f w
...
let qWeakChecker =
  check_normalised_formulas {qnum = Z0; qden = XH}
    {qnum = Zpos XH; qden = XH}
...
let qnegate t0 tg =
  cnf_negate {qnum = Z0; qden = XH}
    {qnum = Zpos XH; qden = XH}
    qplus qmult qminus qopp qeq_bool qle_bool t0 tg

(** val qunsat : q nFormula -> bool **)

let qunsat = check_inconsistent {qnum = Z0; qden = XH} qeq_bool qle_bool

(** val qdeduce : q nFormula -> q nFormula -> q nFormula option **)

let qdeduce = nformula_plus_nformula {qnum = Z0; qden = XH} qplus qeq_bool

(** val normQ : q pExpr -> q pol **)

let normQ =
  norm {qnum = Z0; qden = XH}
    {qnum = Zpos XH; qden = XH}
...
let cnfQ f = rxcnf qunsat qdeduce qnormalise qnegate true f

(** val qTautoChecker : q formula bFormula -> qWitness list -> bool **)

let qTautoChecker f w =
  tauto_checker qunsat qdeduce qnormalise qnegate
    (fun cl -> qWeakChecker (map fst cl))
    f w
...
let z_of_exp = function Inl z1 -> z1 | Inr n0 -> Z.of_nat n0
...
let rec q_of_Rcst = function
  ...
  | CZ z0 ->
    {qnum = z0; qden = XH}
  | CPlus (r1, r2) ->
    qplus (q_of_Rcst r1) (q_of_Rcst r2)
  | CMinus (r1, r2) ->
    qminus (q_of_Rcst r1) (q_of_Rcst r2)
  | CMult (r1, r2) ->
    qmult (q_of_Rcst r1) (q_of_Rcst r2)
  | CPow (r1, z0) ->
    qpower (q_of_Rcst r1) (z_of_exp z0)
  | CInv r0 ->
    qinv (q_of_Rcst r0)
  | COpp r0 ->
    qopp (q_of_Rcst r0)
...
let runsat = check_inconsistent {qnum = Z0; qden = XH} qeq_bool qle_bool

(** val rdeduce : q nFormula -> q nFormula -> q nFormula option **)

let rdeduce = nformula_plus_nformula {qnum = Z0; qden = XH} qplus qeq_bool

(** val rTautoChecker : rcst formula bFormula -> rWitness list -> bool **)

let rTautoChecker f w =
  tauto_checker runsat rdeduce rnormalise rnegate
    (fun cl -> rWeakChecker (map fst cl))
    (map_bformula (map_Formula q_of_Rcst) f)
    w
