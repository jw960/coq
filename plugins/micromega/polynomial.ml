  ...
  let pp o m =
    ...
    let rec pp_list o l =
      ...
      | [e] ->
        pp_elt o e
      | e :: l ->
        Printf.fprintf o "%a*%a" pp_elt e pp_list l
    in
...
let pp_mon o (m, i) =
  if Monomial.is_const m then
    ...
    | Int 1 ->
      Monomial.pp o m
    | Int -1 ->
      Printf.fprintf o "-%a" Monomial.pp m
    | Int 0 ->
      ()
    | _ ->
      Printf.fprintf o "%s*%a" (string_of_num i) Monomial.pp m
      ...
      (fun acc x v ->
        if p v then
          ...
          | None ->
            acc
          | Some x ->
            if is_linear_for x l then x :: acc else acc
        else acc)
      [] l
...
module ProofFormat = struct
  ...
  let rec output_prf_rule o = function
    | Annot (s, p) ->
      Printf.fprintf o "(%a)@%s" output_prf_rule p s
    | Hyp i ->
      Printf.fprintf o "Hyp %i" i
    | Def i ->
      Printf.fprintf o "Def %i" i
    | Cst c ->
      Printf.fprintf o "Cst %s" (string_of_num c)
    | Zero ->
      Printf.fprintf o "Zero"
    | Square s ->
      Printf.fprintf o "(%a)^2" Poly.pp (LinPoly.pol_of_linpol s)
    | MulC (p, pr) ->
    ...
    | AddPrf (p1, p2) ->
      Printf.fprintf o "%a + %a" output_prf_rule p1 output_prf_rule p2
    | CutPrf p ->
      Printf.fprintf o "[%a]" output_prf_rule p
    | Gcd (c, p) ->
  ...
  let rec proof_max_id = function
    | Done ->
      -1
    | Step (i, pr, prf) ->
      max i (max (pr_rule_max_id pr) (proof_max_id prf))
    | Enum (i, p1, _, p2, l) ->
  ...
  let rec pr_rule_def_cut id = function
    ...
    | Gcd (c, p) ->
      let bds, id, p = pr_rule_def_cut id p in
      ((id, p) :: bds, id + 1, Def id)
    | (Square _ | Cst _ | Def _ | Hyp _ | Zero) as x ->
      ([], id, x)
  ...
  let rec pr_rule_collect_hyps pr =
    match pr with
    | Annot (_, pr) ->
      pr_rule_collect_hyps pr
    | Hyp i | Def i ->
      ISet.add i ISet.empty
    | Cst _ | Zero | Square _ ->
      ISet.empty
    | MulC (_, pr) | Gcd (_, pr) | CutPrf pr ->
      pr_rule_collect_hyps pr
    | MulPrf (p1, p2) | AddPrf (p1, p2) ->
  ...
  let simplify_proof p =
    let rec simplify_proof p =
      ...
      | Done ->
        (Done, ISet.empty)
      | Step (i, pr, Done) ->
        (p, ISet.add i (pr_rule_collect_hyps pr))
      | Step (i, pr, prf) ->
  ...
  let rec normalise_proof id prf =
    ...
    | Done ->
      (id, Done)
    | Step (i, Gcd (c, p), Done) ->
      normalise_proof id (Step (i, p, Done))
    | Step (i, p, prf) ->
      let bds, id, p' = pr_rule_def_cut id p in
  ...
  module OrdPrfRule = struct
    ...
    let rec compare p1 p2 =
      ...
      | CutPrf p, CutPrf p' ->
        compare p p'
      | _, _ ->
        Util.pervasives_compare (id_of_constr p1) (id_of_constr p2)
  end
  ...
  let rec mul_cst_proof c p =
    match p with
    | Annot (s, p) ->
      Annot (s, mul_cst_proof c p)
    | MulC (v, p') ->
      MulC (Vect.mul c v, p')
    | _ -> (
      match sign_num c with
      | 0 ->
        Zero (* This is likely to be a bug *)
      | -1 ->
        MulC (LinPoly.constant c, p) (* [p] should represent an equality *)
      | 1 ->
        if eq_num (Int 1) c then p else MulPrf (Cst c, p)
      | _ ->
        assert false )
  ...
  let rec dev_prf_rule p =
    ...
    | MulPrf (p1, p2) -> (
      ...
      match p1'' with
      | Cst c ->
        PrfRuleMap.map (fun v1 -> Vect.mul c v1) p2'
      | _ ->
 ...
 *)
  ...
  module Env = struct
    let rec string_of_int_list l =
      ...
      | [] ->
        ""
      | i :: l ->
        Printf.sprintf "%i,%s" i (string_of_int_list l)
    ...
    let id_of_hyp hyp l =
      let rec xid_of_hyp i l' =
        ...
        | hyp' :: l' ->
          if hyp = hyp' then i else xid_of_hyp (i + 1) l'
      in
      xid_of_hyp 0 l
  end
  ...
  let cmpl_prf_rule norm (cst : num -> 'a) env prf =
    let rec cmpl = function
      | Annot (s, p) ->
        cmpl p
      | Hyp i | Def i ->
        Mc.PsatzIn (CamlToCoq.nat (Env.id_of_hyp i env))
      | Cst i ->
        Mc.PsatzC (cst i)
      | Zero ->
        Mc.PsatzZ
      | MulPrf (p1, p2) ->
        Mc.PsatzMulE (cmpl p1, cmpl p2)
      | AddPrf (p1, p2) ->
        Mc.PsatzAdd (cmpl p1, cmpl p2)
      | MulC (lp, p) ->
        let lp = norm (LinPoly.coq_poly_of_linpol cst lp) in
        Mc.PsatzMulC (lp, cmpl p)
      | Square lp ->
        Mc.PsatzSquare (norm (LinPoly.coq_poly_of_linpol cst lp))
      | _ ->
        failwith "Cuts should already be compiled"
    in
    cmpl prf
  ...
  let rec cmpl_proof env = function
    ...
    | Step (i, p, prf) -> (
      ...
      | _ ->
        Mc.RatProof (cmpl_prf_rule_z env p, cmpl_proof (i :: env) prf) )
    | Enum (i, p1, _, p2, l) ->
      Mc.EnumProof
    ...
    | Cst n -> (
      ...
      , match Num.compare_num n (Int 0) with
        ...
        | 1 ->
          Gt
        | _ ->
          failwith "eval_prf_rule : negative constant" ) )
    | Zero ->
      (Vect.null, Ge)
    | Square v ->
      (LinPoly.product v v, Ge)
    | MulC (v, p) -> (
      let p1, o = eval_prf_rule env p in
      match o with
      | Eq ->
        (LinPoly.product v p1, Eq)
      | _ ->
  ...
  let rec eval_proof env p =
    match p with
    | Done ->
      failwith "Proof is not finished"
    | Step (i, prf, rst) ->
...
module WithProof = struct
  ...
  let mult p ((p1, o1), prf1) =
    match o1 with
    | Eq ->
      ((LinPoly.product p p1, o1), ProofFormat.sMulC p prf1)
    | Gt | Ge ->
  ...
  let cutting_plane ((p, o), prf) =
    ...
    then None (* Nothing to do *)
      ...
      if c1 =/ c1' then None
        ...
        | Gt ->
          failwith "cutting_plane ignore strict constraints"
        | Ge ->
  ...
  let construct_sign p =
    ...
    if Vect.is_null p' then
      Some
        ( match sign_num c with
        | 0 ->
          (true, Eq, ProofFormat.Zero)
        | 1 ->
          (true, Gt, ProofFormat.Cst c)
        | _ (*-1*) ->
          (false, Gt, ProofFormat.Cst (minus_num c)) )
    else None
  ...
  let rec linear_pivot sys ((lp1, op1), prf1) x ((lp2, op2), prf2) =
    ...
    if Vect.is_null a2 then (* We are done *)
      ...
      | Eq, (Ge | Gt) -> (
        ...
        | None ->
          None (* Impossible to pivot without sign information *)
        | Some (b, o, prf) ->
      ...
      | (Ge | Gt), (Ge | Gt) -> (
        ...
        | _ ->
          None )
      | (Ge | Gt), Eq ->
        failwith "pivot: equality as second argument"
  ...
  let linear_pivot sys ((lp1, op1), prf1) x ((lp2, op2), prf2) =
    ...
    | None ->
      None
    | Some (c, p) ->
      Some (c, ProofFormat.simplify_prf_rule p)
  ...
  let subst sys0 =
    let elim sys =
      ...
      | None ->
        None
      | Some (v, pc) ->
        simplify (linear_pivot sys0 pc v) sys'
    in
