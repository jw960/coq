...
let rec pp_list s f o l =
  ...
  | [e] ->
    f o e
  | e :: l ->
    f o e; output_string o s; pp_list s f o l
...
let rec try_any l x =
  ...
  | [] ->
    None
  | (f, s) :: l -> (
    match f x with None -> try_any l x | x -> x )
...
let rec find_option pred l =
  ...
  | [] ->
    raise Not_found
  | e :: l -> (
    match pred e with Some r -> r | None -> find_option pred l )
...
let denominator = function
  | Int _ | Big_int _ ->
    unit_big_int
  | Ratio r ->
    Ratio.denominator_ratio r

let numerator = function
  | Ratio r ->
    Ratio.numerator_ratio r
  | Int i ->
...
let rec app_funs l x =
  ...
  | [] ->
    None
  | f :: fl -> (
    match f x with None -> app_funs fl x | Some x' -> Some x' )
...
module CoqToCaml = struct
  ...
  let rec positive_big_int p =
    ...
    | XH ->
      unit_big_int
    | XI p ->
      add_int_big_int 1 (mult_int_big_int 2 (positive_big_int p))
    | XO p ->
      mult_int_big_int 2 (positive_big_int p)
  ...
  let z_big_int x =
    ...
    | Zpos p ->
      positive_big_int p
    | Zneg p ->
      minus_big_int (positive_big_int p)
...
module CamlToCoq = struct
  ...
  let bigint x =
    ...
    | 0 ->
      Z0
    | 1 ->
      Zpos (positive_big_int x)
    | _ ->
      Zneg (positive_big_int (minus_big_int x))
...
module Hash = struct
  ...
  let eq_z z1 z2 =
    ...
    | Mc.Z0, Mc.Z0 ->
      true
    | Mc.Zpos p1, Mc.Zpos p2 | Mc.Zneg p1, Mc.Zneg p2 ->
      eq_positive p1 p2
    | _, _ ->
      false
  ...
  let rec eq_pol eq p1 p2 =
    ...
    | Mc.Pc c1, Mc.Pc c2 ->
      eq c1 c2
    | Mc.Pinj (i1, p1), Mc.Pinj (i2, p2) ->
      eq_positive i1 i2 && eq_pol eq p1 p2
  ...
  let hash_pol helt =
    let rec hash acc = function
      | Mc.Pc c ->
        helt (combine acc 1) c
      | Mc.Pinj (p, c) ->
        hash (combine (combine acc 1) (CoqToCaml.index p)) c
      | Mc.PX (p1, i, p2) ->
