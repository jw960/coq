...
module M = struct
  ...
  let rec expr_to_term = function
    | PEc z ->
      Const (C2Ml.q_to_num z)
    | PEX v ->
      Var ("x" ^ string_of_int (C2Ml.index v))
    | PEmul (p1, p2) ->
      ...
      res
    | PEadd (p1, p2) ->
      Add (expr_to_term p1, expr_to_term p2)
    | PEsub (p1, p2) ->
      Sub (expr_to_term p1, expr_to_term p2)
    | PEpow (p, n) ->
      Pow (expr_to_term p, C2Ml.n n)
    | PEopp p ->
      Opp (expr_to_term p)
end
  ...
  let rec f i = function
    ...
    | (e, k) :: l -> (
      ...
      | Mc.Equal ->
        ((e, i) :: eq, ge, neq)
      | Mc.NonStrict ->
        (eq, (e, Axiom_le i) :: ge, neq)
      | Mc.Strict ->
        (* e > 0 == e >= 0 /\ e <> 0 *)
        (eq, (e, Axiom_lt i) :: ge, (e, Axiom_lt i) :: neq)
      | Mc.NonEqual ->
        (eq, ge, (e, Axiom_eq i) :: neq) )
...
let run_prover prover pb =
  match prover with
  | "real_nonlinear_prover", Some d ->
    real_nonlinear_prover d pb
  | "pure_sos", None ->
    pure_sos pb
  | prover, _ ->
