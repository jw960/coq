...
let find_pivot vr (rst : Restricted.t) tbl =
  ...
  | Some mx ->
    Done (Max mx) (* Maximum is reached; we are done *)
  | None -> (
    (* Extract the vector *)
    ...
    if (not opt) && Vect.get_cst (IMap.find vr tbl) >=/ Int 0 then
      ...
      | Done r -> (
        ...
        | Ubnd x ->
          ...
          Opt (t', r)
        | Feas ->
          raise (Invalid_argument "find_pivot") )
      | Pivot (i, j, s) ->
        if debug then begin
...
let optimise obj l =
  ...
  let bound pos res =
    match res with
    | Opt (_, Max n) ->
      Some (if pos then n else minus_num n)
    | Opt (_, Ubnd _) ->
      None
    | Opt (_, Feas) ->
      None
