...
let rec pp_proof o prf =
  match prf with
  | Assum i ->
    Printf.fprintf o "H%i" i
  | Elim (v, prf1, prf2) ->
    Printf.fprintf o "E(%i,%a,%a)" v pp_proof prf1 pp_proof prf2
  | And (prf1, prf2) ->
    Printf.fprintf o "A(%a,%a)" pp_proof prf1 pp_proof prf2
...
let pp_cstr o (vect, bnd) =
  ...
  | None ->
    ()
  | Some n ->
    Printf.fprintf o "%s <= " (string_of_num n) );
  Vect.pp o vect;
  match r with
  | None ->
    output_string o "\n"
...
let merge_cstr_info i1 i2 =
  ...
  | None ->
    None (* Could directly raise a system contradiction exception *)
  | Some bnd ->
...
let xadd_cstr vect cstr_info sys =
  try
    ...
    | None ->
      raise (SystemContradiction (And (cstr_info.prf, !info.prf)))
    | Some info' ->
      info := info'
  ...
  let vars =
    List.fold_left
      (fun vrs (cstr, i) ->
        ...
        | Contradiction ->
          raise (SystemContradiction (Assum i))
        | Redundant ->
          vrs
        | Cstr (vect, info) ->
  ...
  | vl ->
    ...
    let cons_bound lst bd =
      ...
      | None ->
        lst
      | Some bnd ->
        (vl, vect, {info with bound = (Some bnd, None)}) :: lst
    in
      ...
      List.iter
        (fun r_elem ->
          ...
          | Redundant ->
            ()
          | Contradiction ->
            raise (SystemContradiction info.prf)
          | Cstr (vect, info) ->
            xadd_cstr vect info new_sys)
        r)
    l;
...
let pick_small_value bnd =
  ...
  | None, None ->
    Int 0
  | None, Some i ->
    if Int 0 <=/ floor_num i then Int 0 else floor_num i
  | Some i, None ->
    if i <=/ Int 0 then Int 0 else ceiling_num i
  | Some i, Some j ->
...
module Fourier = struct
  let optimise vect l =
    ...
    | Inr prf ->
      None (* This is an unsatisfiability proof *)
    | Inl (s, _) -> (
...
*)
  ...
  let normalise_proofs l =
    List.fold_left
      (fun acc (prf, cstr) ->
        match acc with
        | Inr _ ->
          acc (* I already found a contradiction *)
        | Inl acc -> (
    ...
    | Some (prfl, cstrl, l), Some (prfr, cstrr, r) -> (
      if l <=/ r then Inl (oleft, oright)
        ...
        | Some (v, _, _) -> (
          ...
          | None ->
            failwith "merge_proof : pivot is not possible"
          | Some x ->
            Inr x ) )
  ...
  *)
    let rec mk_proof prf =
      match prf with
      | Assum i ->
        [(Vect.set i (Int 1) Vect.null, List.nth hyps i)]
      | Elim (v, prf1, prf2) ->
          ...
          let prfs =
            List.fold_left
              (fun acc e ->
                match acc with
                | Inr _ ->
                  acc (* I have a contradiction *)
                | Inl (oleft, oright) ->
                  merge_proof oleft e oright)
              (Inl (None, None))
              l
          in
          match prfs with
