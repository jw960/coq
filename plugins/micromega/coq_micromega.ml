...
let rec pp_formula o (f : 'cst formula) =
  Mc.(
    ...
    | X c ->
      output_string o "X "
    | A (_, (t, _)) ->
      Printf.fprintf o "A(%a)" Tag.pp t
    | Cj (f1, f2) ->
      Printf.fprintf o "C(%a,%a)" pp_formula f1 pp_formula f2
    | D (f1, f2) ->
      Printf.fprintf o "D(%a,%a)" pp_formula f1 pp_formula f2
    | I (f1, n, f2) ->
      Printf.fprintf o "I(%a,%s,%a)" pp_formula f1
        (match n with Some id -> Names.Id.to_string id | None -> "")
        pp_formula f2
    | N f ->
      Printf.fprintf o "N(%a)" pp_formula f)
...
module M = struct
  ...
  let rec dump_nat x =
    ...
    | Mc.O ->
      Lazy.force coq_O
    | Mc.S p ->
      EConstr.mkApp (Lazy.force coq_S, [|dump_nat p|])

  let rec parse_positive sigma term =
    let i, c = get_left_construct sigma term in
    match i with
    | 1 ->
      Mc.XI (parse_positive sigma c.(0))
    | 2 ->
      Mc.XO (parse_positive sigma c.(0))
    | 3 ->
      Mc.XH
    | i ->
      raise ParseError
  ...
  let rec dump_positive x =
    ...
    | Mc.XH ->
      Lazy.force coq_xH
    | Mc.XO p ->
      EConstr.mkApp (Lazy.force coq_xO, [|dump_positive p|])
    | Mc.XI p ->
      EConstr.mkApp (Lazy.force coq_xI, [|dump_positive p|])
  ...
  let dump_n x =
    ...
    | Mc.N0 ->
      Lazy.force coq_N0
    | Mc.Npos p ->
      EConstr.mkApp (Lazy.force coq_Npos, [|dump_positive p|])
  ...
  let parse_z sigma term =
    ...
    | 1 ->
      Mc.Z0
    | 2 ->
      Mc.Zpos (parse_positive sigma c.(0))
    | 3 ->
      Mc.Zneg (parse_positive sigma c.(0))
    | i ->
      raise ParseError
  ...
  let dump_z x =
    ...
    | Mc.Z0 ->
      Lazy.force coq_ZERO
    | Mc.Zpos p ->
      EConstr.mkApp (Lazy.force coq_POS, [|dump_positive p|])
    | Mc.Zneg p ->
      EConstr.mkApp (Lazy.force coq_NEG, [|dump_positive p|])
  ...
  let rec pp_Rcst o cst =
    ...
    | Mc.C1 ->
      output_string o "C1"
    | Mc.CQ q ->
      output_string o "CQ _"
    | Mc.CZ z ->
      pp_z o z
    | Mc.CPlus (x, y) ->
      Printf.fprintf o "(%a + %a)" pp_Rcst x pp_Rcst y
    | Mc.CMinus (x, y) ->
      Printf.fprintf o "(%a - %a)" pp_Rcst x pp_Rcst y
    | Mc.CMult (x, y) ->
      Printf.fprintf o "(%a * %a)" pp_Rcst x pp_Rcst y
    | Mc.CPow (x, y) ->
      Printf.fprintf o "(%a ^ _)" pp_Rcst x
    | Mc.CInv t ->
      Printf.fprintf o "(/ %a)" pp_Rcst t
    | Mc.COpp t ->
      Printf.fprintf o "(- %a)" pp_Rcst t
  ...
  let rec dump_Rcst cst =
    ...
    | Mc.C1 ->
      Lazy.force coq_C1
    | Mc.CQ q ->
      EConstr.mkApp (Lazy.force coq_CQ, [|dump_q q|])
    | Mc.CZ z ->
      EConstr.mkApp (Lazy.force coq_CZ, [|dump_z z|])
    | Mc.CPlus (x, y) ->
    ...
    | Mc.CInv t ->
      EConstr.mkApp (Lazy.force coq_CInv, [|dump_Rcst t|])
    | Mc.COpp t ->
      EConstr.mkApp (Lazy.force coq_COpp, [|dump_Rcst t|])
  ...
  let pp_list op cl elt o l =
    let rec _pp o l =
      ...
      | [] ->
        ()
      | [e] ->
        Printf.fprintf o "%a" elt e
      | e :: l ->
        Printf.fprintf o "%a ,%a" elt e _pp l
    in
  ...
  let dump_expr typ dump_z e =
    let rec dump_expr e =
      match e with
      | Mc.PEX n ->
        EConstr.mkApp (Lazy.force coq_PEX, [|typ; dump_var n|])
      | Mc.PEc z ->
        EConstr.mkApp (Lazy.force coq_PEc, [|typ; dump_z z|])
      | Mc.PEadd (e1, e2) ->
      ...
      | Mc.PEopp e ->
        EConstr.mkApp (Lazy.force coq_PEopp, [|typ; dump_expr e|])
      | Mc.PEmul (e1, e2) ->
  ...
  let dump_pol typ dump_c e =
    let rec dump_pol e =
      match e with
      | Mc.Pc n ->
        EConstr.mkApp (Lazy.force coq_Pc, [|typ; dump_c n|])
      | Mc.Pinj (p, pol) ->
        EConstr.mkApp
  ...
  let dump_psatz typ dump_z e =
    ...
    let rec dump_cone e =
      match e with
      | Mc.PsatzIn n ->
        EConstr.mkApp (Lazy.force coq_PsatzIn, [|z; dump_nat n|])
      | Mc.PsatzMulC (e, c) ->
        EConstr.mkApp
      ...
      | Mc.PsatzC p ->
        EConstr.mkApp (Lazy.force coq_PsatzC, [|z; dump_z p|])
      | Mc.PsatzZ ->
        EConstr.mkApp (Lazy.force coq_PsatzZ, [|z|])
    in
    dump_cone e
  ...
  let parse_zop gl (op, args) =
    ...
    match args with
    | [|a1; a2|] ->
      (assoc_const sigma op zop_table, a1, a2)
    | [|ty; a1; a2|] ->
      if
  ...
  let parse_rop gl (op, args) =
    ...
    match args with
    | [|a1; a2|] ->
      (assoc_const sigma op rop_table, a1, a2)
    | [|ty; a1; a2|] ->
      if
        EConstr.eq_constr sigma op (Lazy.force coq_Eq)
 ...
 *)
  ...
  let dump_formula typ dump_atom f =
    ...
    let rec xdump f =
      ...
      | Mc.FF ->
        app_ctor coq_FF []
      | Mc.Cj (x, y) ->
        app_ctor coq_And [xdump x; xdump y]
      | Mc.D (x, y) ->
        app_ctor coq_Or [xdump x; xdump y]
      | Mc.I (x, _, y) ->
        app_ctor coq_Impl
          [ xdump x
          ...
          ; xdump y ]
      | Mc.N x ->
        app_ctor coq_Neg [xdump x]
      | Mc.A (x, _) ->
        app_ctor coq_Atom [dump_atom x; Lazy.force coq_tt]
      | Mc.X t ->
        app_ctor coq_X [t]
    in
    xdump f
    ...
    Mc.(
      let rec doit env = function
        | TT | FF | A (_, _) ->
          env
        | X t ->
          fst (Env.compute_rank_add env t)
  ...
  let rec dump_Rcst_as_R cst =
    ...
    | Mc.C1 ->
      Lazy.force coq_R1
    | Mc.CQ q ->
      EConstr.mkApp (Lazy.force coq_IQR, [|dump_q q|])
    | Mc.CZ z ->
      EConstr.mkApp (Lazy.force coq_IZR, [|dump_z z|])
    | Mc.CPlus (x, y) ->
      EConstr.mkApp
    ...
    | Mc.CInv t ->
      EConstr.mkApp (Lazy.force coq_Rinv, [|dump_Rcst_as_R t|])
    | Mc.COpp t ->
      EConstr.mkApp (Lazy.force coq_Ropp, [|dump_Rcst_as_R t|])
  ...
  let make_goal_of_formula gl dexpr form =
    ...
    let dump_expr i e =
      let rec dump_expr = function
        ...
        | Mc.PEopp e ->
          EConstr.mkApp (dexpr.dump_opp, [|dump_expr e|])
        | Mc.PEmul (e1, e2) ->
...
let rec dump_varmap typ m =
  match m with
  | Mc.Empty ->
    EConstr.mkApp (Lazy.force coq_Empty, [|typ|])
  | Mc.Elt v ->
    EConstr.mkApp (Lazy.force coq_Elt, [|typ; v|])
  | Mc.Branch (l, o, r) ->
    EConstr.mkApp
...
let rec size_of_psatz = function
  | Micromega.PsatzIn _ ->
    1
  | Micromega.PsatzSquare _ ->
    1
  | Micromega.PsatzMulC (_, p) ->
    1 + size_of_psatz p
...
let rec size_of_pf = function
  | Micromega.DoneProof ->
    1
  | Micromega.RatProof (p, a) ->
    size_of_pf a + size_of_psatz p
  | Micromega.CutProof (p, a) ->
    size_of_pf a + size_of_psatz p
  ...
  let compact_proof (old_cl : 'cst clause) (prf, prover) (new_cl : 'cst clause)
    ...
    let res =
      ...
      with x when CErrors.noncritical x -> (
        ...
        | Unknown | Model _ ->
          failwith "proof compaction error"
        | Prf p ->
          p )
    in
    if debug then begin
...
let rec abstract_wrt_formula f1 f2 =
  Mc.(
    ...
    | TT, TT ->
      TT
    | N x, N y ->
      N (abstract_wrt_formula x y)
    | _ ->
      failwith "abstract_wrt_formula")
...
Tacticals.New.tclTHEN
      ...
      with
      | Mfourier.TimeOut ->
        Tacticals.New.tclFAIL 0 (Pp.str "Timeout")
      | CsdpNotFound ->
        flush stdout;
...
let lift_ratproof prover l =
  ...
  | Unknown | Model _ ->
    Unknown
  | Prf c ->
    Prf (Mc.RatProof (c, Mc.DoneProof))
...
let rec z_to_q_pol e =
  match e with
  | Mc.Pc z ->
    Mc.Pc {Mc.qnum = z; Mc.qden = Mc.XH}
  | Mc.Pinj (p, pol) ->
    Mc.Pinj (p, z_to_q_pol pol)
  | Mc.PX (pol1, p, pol2) ->
    Mc.PX (z_to_q_pol pol1, p, z_to_q_pol pol2)
...
let xhyps_of_cone base acc prf =
  let rec xtract e acc =
    match e with
    | Mc.PsatzC _ | Mc.PsatzZ | Mc.PsatzSquare _ ->
      acc
    | Mc.PsatzIn n ->
...
let compact_cone prf f =
  ...
  let rec xinterp prf =
    ...
    | Mc.PsatzIn n ->
      Mc.PsatzIn (np n)
    | Mc.PsatzMulC (e, c) ->
      Mc.PsatzMulC (e, xinterp c)
    | Mc.PsatzAdd (e1, e2) ->
      Mc.PsatzAdd (xinterp e1, xinterp e2)
    | Mc.PsatzMulE (e1, e2) ->
      Mc.PsatzMulE (xinterp e1, xinterp e2)
  in
  xinterp prf
...
let hyps_of_pt pt =
  let rec xhyps base pt acc =
    ...
    | Mc.DoneProof ->
      acc
    | Mc.RatProof (c, pt) ->
      xhyps (base + 1) pt (xhyps_of_cone base acc c)
    | Mc.CutProof (c, pt) ->
      xhyps (base + 1) pt (xhyps_of_cone base acc c)
