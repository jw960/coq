...
let dev_form n_spec p =
  let rec dev_form p =
    match p with
    | Mc.PEc z ->
      Poly.constant (n_spec.number_to_num z)
    | Mc.PEX v ->
      Poly.variable (C2Ml.positive v)
    | Mc.PEmul (p1, p2) ->
      let p1 = dev_form p1 in
      let p2 = dev_form p2 in
      Poly.product p1 p2
    | Mc.PEadd (p1, p2) ->
      Poly.addition (dev_form p1) (dev_form p2)
    | Mc.PEopp p ->
      Poly.uminus (dev_form p)
    | Mc.PEsub (p1, p2) ->
...
let dual_raw_certificate l =
  ...
  try
    ...
    | Some cert -> (
      ...
      | None ->
        failwith "dual_raw_certificate: empty_certificate"
      | Some _ ->
...
let linear_prover_cstr sys =
  ...
  | None ->
    None
  | Some cert ->
    Some (ProofFormat.proof_of_farkas (env_of_list prfi) cert)
...
let check_int_sat (cstr, prf) =
  ...
  | _ -> (
    ...
    if eq_num gcd (Int 1) then Normalise (cstr, prf)
      ...
      | Gt ->
        failwith "check_sat : Unexpected operator" )
...
let elim_simple_linear_equality sys0 =
  let elim sys =
    ...
    | None ->
      None
    | Some (v, pc) ->
      simplify (WithProof.linear_pivot sys0 pc v) sys'
  in
...
let rec scale_term t =
  ...
  | Zero ->
    (unit_big_int, Zero)
  | Const n ->
    (denominator n, Const (Big_int (numerator n)))
  | Var n ->
    (unit_big_int, Var n)
  | Opp t ->
  ...
  | Sub _ ->
    failwith "scale term: not implemented"
  | Mul (y, z) ->
...
let rec scale_certificate pos =
  match pos with
  | Axiom_eq i ->
    (unit_big_int, Axiom_eq i)
  | Axiom_le i ->
    (unit_big_int, Axiom_le i)
  | Axiom_lt i ->
    (unit_big_int, Axiom_lt i)
  | Monoid l ->
    (unit_big_int, Monoid l)
  | Rational_eq n ->
    (denominator n, Rational_eq (Big_int (numerator n)))
  | Rational_le n ->
    (denominator n, Rational_le (Big_int (numerator n)))
  | Rational_lt n ->
    (denominator n, Rational_lt (Big_int (numerator n)))
  | Square t ->
    let s, t' = scale_term t in
...
let rec term_to_q_expr = function
  ...
  | Mul (p1, p2) ->
    PEmul (term_to_q_expr p1, term_to_q_expr p2)
  | Add (p1, p2) ->
    PEadd (term_to_q_expr p1, term_to_q_expr p2)
  | Opp p ->
    PEopp (term_to_q_expr p)
  | Pow (t, n) ->
    PEpow (term_to_q_expr t, Ml2C.n n)
  | Sub (t1, t2) ->
    PEsub (term_to_q_expr t1, term_to_q_expr t2)
...
let rec product l =
  ...
  | [] ->
    Mc.PsatzZ
  | [i] ->
    Mc.PsatzIn (Ml2C.nat i)
  | i :: l ->
    Mc.PsatzMulE (Mc.PsatzIn (Ml2C.nat i), product l)
...
let q_cert_of_pos pos =
  let rec _cert_of_pos = function
    ...
    | Square t ->
      Mc.PsatzSquare (term_to_q_pol t)
    | Eqmul (t, y) ->
      Mc.PsatzMulC (term_to_q_pol t, _cert_of_pos y)
    | Sum (y, z) ->
      Mc.PsatzAdd (_cert_of_pos y, _cert_of_pos z)
    | Product (y, z) ->
      Mc.PsatzMulE (_cert_of_pos y, _cert_of_pos z)
  in
  simplify_cone q_spec (_cert_of_pos pos)

let rec term_to_z_expr = function
  | Const n ->
    PEc (Ml2C.bigint (big_int_of_num n))
  | Zero ->
    PEc Z0
  | Var s ->
    PEX (Ml2C.index (int_of_string (String.sub s 1 (String.length s - 1))))
  | Mul (p1, p2) ->
    PEmul (term_to_z_expr p1, term_to_z_expr p2)
  | Add (p1, p2) ->
    PEadd (term_to_z_expr p1, term_to_z_expr p2)
  | Opp p ->
    PEopp (term_to_z_expr p)
  | Pow (t, n) ->
    PEpow (term_to_z_expr t, Ml2C.n n)
  | Sub (t1, t2) ->
    PEsub (term_to_z_expr t1, term_to_z_expr t2)
...
let z_cert_of_pos pos =
  ...
  let rec _cert_of_pos = function
    ...
    | Square t ->
      Mc.PsatzSquare (term_to_z_pol t)
    | Eqmul (t, y) ->
  ...
  let rec xextract2 rl l =
    ...
    | e :: l -> (
      ...
      | None, _ ->
        xextract2 (e :: rl) l
      | Some (r, e'), l' ->
        (Some (r, e, e'), List.rev_append rl l') )
  in
  xextract2 [] l
...
let get_bound sys =
  ...
  | Some (lb, e, ub) -> (
    ...
    | _ ->
      failwith "Interval without proof" )
  | None ->
    None
...
let xlia (can_enum : bool) reduction_equations sys =
  let rec enum_proof (id : int) (sys : prf_sys) =
    ...
    | None ->
      Unknown (* Is the systeme really unbounded ? *)
  ...
  and aux_lia (id : int) (sys : prf_sys) =
    ...
    try
      ...
      | None ->
        if can_enum then enum_proof id sys else Unknown
    with FoundProof prf ->
