...
module MakeTable (E : Elt) = struct
  ...
  let register_hint evd t elt =
    ...
    | _ ->
      E.table := HConstr.add t (OtherTerm t, E.cast elt) !E.table
...
let locate_const str =
  ...
  | GlobRef.ConstRef c ->
    c
  | _ ->
    CErrors.anomaly Pp.(str rf ++ str " should be a constant")
...
let constr_of_texpr = function
  | Constant (i, e) | Var (i, e) ->
    if i.isid then Some e else None
  | _ ->
    None
...
let inj_term_of_texpr evd = function
  | Injterm e ->
    e
  | Var (inj, e) ->
    mkvar false evd inj e
  | Constant (inj, e) ->
    mkvar true evd inj e
...
let mkapp_id evd i inj (unop, u) f e1 =
  EUnOpT.(
    if EConstr.eq_constr evd u.unop f then
      ...
      match e1 with
      | Constant (_, e) | Var (_, e) ->
        Var (inj, EConstr.mkApp (f, [|e|]))
      | Injterm e1 ->
        Injterm
          (EConstr.mkApp
             ( force mkapp
...
let is_progress_rewrite evd t rew =
  ...
  | _ ->
    failwith "is_progress_rewrite: not even an application"
