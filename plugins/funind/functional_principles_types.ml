...
let compute_new_princ_type_from_rel rel_to_fun sorts princ_type =
  ...
  let rec change_predicates_names (avoid : Id.t list)
    ...
    | decl :: predicates -> (
      ...
      | Anonymous ->
        anomaly (Pp.str "Anonymous property binder.") )
  in
  ...
  let change_predicate_sort i decl =
    ...
    let args, _ =
      decompose_prod (EConstr.Unsafe.to_constr (RelDecl.get_type decl))
    in
    let real_args =
  ...
  let rel_as_kn =
    fst
      ...
      | Some (GlobRef.IndRef ind) ->
        ind
      | _ ->
        user_err Pp.(str "Not a valid predicate") )
  in
  ...
  let rec compute_new_princ_type remove env pre_princ : types * constr list =
    let ((new_princ_type, _) as res) =
      ...
      | (Ind _ | Construct _) when is_dom pre_princ ->
        raise Toberemoved
