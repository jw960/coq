...
let change_vars =
  let rec change_vars mapping rt =
    DAst.map_with_loc
      (fun ?loc -> function GRef _ as x -> x | GVar id ->
        ...
        | GLetTuple (nal, (na, rto), b, e) ->
          ...
          GLetTuple
            ...
            , change_vars new_mapping e ) | GCases (sty, infos, el, brl) ->
          GCases
            ( sty
            , infos
        ...
        | GIf (b, (na, e_option), lhs, rhs) ->
          GIf
            ...
            , change_vars mapping rhs ) | GRec _ ->
          user_err ?loc Pp.(str "Local (co)fixes are not supported")
        | GSort _ as x -> x | GHole _ as x -> x | GInt _ as x -> x
        | GFloat _ as x -> x | GCast (b, c) ->
          GCast
            ( change_vars mapping b
...
let rec alpha_rt excluded rt =
  ...
  let new_rt =
    ...
    @@
    match DAst.get rt with
    | (GRef _ | GVar _ | GEvar _ | GPatVar _) as rt ->
      rt
    ...
    | GRec _ ->
      user_err Pp.(str "Not handled GRec")
    | (GSort _ | GInt _ | GFloat _ | GHole _) as rt ->
      rt
    | GCast (b, c) ->
  ...
  let rec is_free_in x =
    DAst.with_loc_val
      (fun ?loc -> function GRef _ -> false | GVar id' -> Id.compare id' id == 0
        | GEvar _ -> false | GPatVar _ -> false | GApp (rt, rtl) ->
          List.exists is_free_in (rt :: rtl)
        | GLambda (n, _, t, b) | GProd (n, _, t, b) ->
          let check_in_b =
            match n with Name id' -> not (Id.equal id' id) | _ -> true
          in
          is_free_in t || (check_in_b && is_free_in b) | GLetIn (n, b, t, c) ->
          let check_in_c =
            match n with Name id' -> not (Id.equal id' id) | _ -> true
          in
          is_free_in b
          || Option.cata is_free_in true t
          || (check_in_c && is_free_in c) | GCases (_, _, el, brl) ->
          List.exists (fun (e, _) -> is_free_in e) el
          || List.exists is_free_in_br brl | GLetTuple (nal, _, b, t) ->
          let check_in_nal =
            not
              (List.exists
        ...
        | GIf (cond, _, br1, br2) ->
          is_free_in cond || is_free_in br1 || is_free_in br2 | GRec _ ->
          user_err Pp.(str "Not handled GRec") | GSort _ -> false | GHole _ ->
          false | GCast (b, (CastConv t | CastVM t | CastNative t)) ->
  ...
  let rec ids_of_pat ids =
    DAst.with_val (function
      ...
      | PatVar (Name id) ->
        Id.Set.add id ids
      | PatCstr (_, patl, _) ->
        List.fold_left ids_of_pat ids patl)
  in
...
let expand_as =
  let rec add_as map rt =
    ...
    | PatCstr (_, patl, _) ->
      List.fold_left add_as map patl
  in
  let rec expand_as map =
    DAst.map (function
      ...
      | GApp (f, args) ->
        GApp (expand_as map f, List.map (expand_as map) args)
      | GLambda (na, k, t, b) ->
        GLambda (na, k, expand_as map t, expand_as map b)
      | GProd (na, k, t, b) ->
        GProd (na, k, expand_as map t, expand_as map b)
      | GLetIn (na, v, typ, b) ->
        GLetIn
  ...
  let rec change rt =
    ...
    | GHole (ImplicitArg (grk, pk, bk), _, _) -> (
      ...
      with Found evi -> (
        ...
        | Evar_empty ->
          rt (* the hole was not solved : we do nothing *) ) )
    | GHole (BinderType na, _, _) ->
      ...
      res
    | _ ->
      Glob_ops.map_glob_constr change rt
  in
  change rt
