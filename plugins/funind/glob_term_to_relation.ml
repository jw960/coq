...
let ids_of_binder = function
  | LetIn Anonymous | Prod Anonymous | Lambda Anonymous ->
    Id.Set.empty
  | LetIn (Name id) | Prod (Name id) | Lambda (Name id) ->
    Id.Set.singleton id
...
let rec build_entry_lc env sigma funnames avoid rt :
  ...
  | GApp (_, _) -> (
    ...
    | GProd _ ->
      user_err Pp.(str "Cannot apply a type")
    | GInt _ ->
      user_err Pp.(str "Cannot apply an integer")
    | GFloat _ ->
      user_err Pp.(str "Cannot apply a float")
  ...
  | GLambda (n, _, t, b) ->
    ...
    let new_n =
      ...
      | Name _ ->
        n
      | Anonymous ->
        Name (Indfun_common.fresh_id [] "_x")
    in
  ...
  | GLetIn (n, v, typ, b) ->
    ...
    let v =
      ...
      | None ->
        v
      | Some t ->
        DAst.make ?loc:rt.loc @@ GCast (v, CastConv t)
    in
      ...
      List.map2
        (fun pat typ avoid pat'_as_term ->
          ...
          List.fold_right
            (fun id acc ->
              let typ_of_id =
                Typing.unsafe_type_of env_with_pat_ids (Evd.from_env env)
                  (EConstr.mkVar id)
              in
              let raw_typ_of_id =
             ...
             List.fold_right
               (fun id acc ->
                 if Id.Set.mem id this_pat_ids then
                   ( Prod (Name id)
                   , let typ_of_id =
                       Typing.unsafe_type_of new_env (Evd.from_env env)
                         (EConstr.mkVar id)
                     in
                     let raw_typ_of_id =
                       Detyping.detype Detyping.Now false Id.Set.empty new_env
...
let rec rebuild_cons env nb_args relname args crossed_types depth rt =
  ...
  | GProd (n, k, t, b) -> (
    ...
    | GApp (eq_as_ref, [ty; rt1; rt2])
      ...
      with Continue -> (
        ...
        | _ ->
          (mkGProd (n, t, new_b), Id.Set.filter not_free_in_t id_to_exclude) ) )
    | _ -> (
  ...
  | GLambda (n, k, t, b) -> (
    ...
    | _ ->
      anomaly (Pp.str "Should not have an anonymous function here.")
  ...
  | _ ->
    (mkGApp (mkGVar relname, args @ [rt]), Id.Set.empty)
...
let rec compute_cst_params relnames params gt =
  DAst.with_val
    (function
      | GRef _ | GVar _ | GEvar _ | GPatVar _ | GInt _ | GFloat _ ->
        params
      | GApp (f, args) -> (
        match DAst.get f with
...
and compute_cst_params_from_app acc (params, rtl) =
  ...
  | _ :: _, [] ->
    assert false (* the rel has at least nargs + 1 arguments ! *)
      ...
      ++ Ppvernac.pr_vernac
           (CAst.make
              Vernacexpr.
                ...
                ; expr =
                    VernacInductive
                      (None, false, Declarations.Finite, repacked_rel_inds) })
      ++ fnl () ++ msg
    in
    observe msg; raise e
  | reraise ->
