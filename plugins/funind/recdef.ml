...
let const_of_ref = function
  | GlobRef.ConstRef kn ->
    kn
  | _ ->
    anomaly (Pp.str "ConstRef expected.")
...
let tclUSER tac is_mes l =
  ...
  let clear_tac =
    ...
    | None ->
      tclIDTAC
    | Some l ->
      tclMAP (fun id -> tclTRY (clear [id])) (List.rev l)
  in
...
let check_not_nested env sigma forbidden e =
  let rec check_not_nested e =
    ...
    | Meta _ | Evar _ | Sort _ ->
      ()
    | Cast (e, _, t) ->
      check_not_nested e; check_not_nested t
    | Prod (_, t, b) ->
      check_not_nested t; check_not_nested b
    | Lambda (_, t, b) ->
      check_not_nested t; check_not_nested b
    | LetIn (_, v, t, b) ->
    ...
    | Fix _ ->
      user_err Pp.(str "check_not_nested : Fix")
    | CoFix _ ->
      user_err Pp.(str "check_not_nested : Fix")
  in
...
let add_vars sigma forbidden e =
  let rec aux forbidden e =
    ...
    | Var x ->
      x :: forbidden
    | _ ->
      EConstr.fold sigma aux forbidden e
  in
  aux forbidden e
...
let rec travel_aux jinfo continuation_tac (expr_info : constr infos) g =
  ...
  | Proj _ ->
    user_err Pp.(str "Function cannot treat projections")
  | LetIn (na, b, t, e) ->
    ...
    travel jinfo new_continuation_tac
      ...
      g
  | Rel _ ->
    anomaly (Pp.str "Free var in goal conclusion!")
  | Prod _ -> (
    try
  ...
  | App _ -> (
    ...
    if EConstr.eq_constr sigma f expr_info.f_constr then
      ...
      | App _ ->
        assert false (* f is coming from a decompose_app *)
      | Const _
      | Construct _
      | Rel _
      | Evar _
      ...
      | _ ->
        anomaly
          ...
          ++ Pp.str "." ) )
  | Cast (t, _, _) ->
    travel jinfo continuation_tac {expr_info with info = t} g
  | Const _
  | Var _
  | Meta _
  | Evar _
  ...
  begin
    try
      let varx, varz =
        ...
        | _, x :: z :: _ when isVar sigma x && isVar sigma z ->
          (x, z)
        | _ ->
          assert false
      in
      let h =
        List.find
          (fun id ->
            ...
            | _, t :: _ ->
              EConstr.eq_constr sigma t varx
            | _ ->
              false)
          hyple
      in
...
let pf_typel l tac =
  let rec aux tys l =
    ...
    | [] ->
      tac (List.rev tys)
    | hd :: tl ->
      pf_type hd (fun ty -> aux (ty :: tys) tl)
  in
  aux [] l
  ...
  | (_, p, _) :: l ->
    observe_tclTHENLIST
      ...
      ; onNLastHypsId 3 (fun lids ->
            match lids with
            | [hle2; hle1; pmax] ->
              compute_max rew_tac (mkVar pmax) l
            | _ ->
              assert false) ]
...
let whole_start concl_tac nb_args is_mes func input_type relation rec_arg_num :
 ...
 fun g ->
  ...
  let f_id =
    ...
    | Name f_id ->
      next_ident_away_in_goal f_id ids
    | Anonymous ->
      anomaly (Pp.str "Anonymous function.")
  in
    ...
    List.fold_left
      (fun (n_ids, ids) (n_name, _) ->
        ...
        | Name id ->
          ...
          (n_id :: n_ids, n_id :: ids)
        | _ ->
          anomaly (Pp.str "anonymous argument."))
      ([], f_id :: ids)
      n_names_types
  in
...
let clear_goals sigma =
  let rec clear_goal t =
    ...
    | _ ->
      EConstr.map sigma clear_goal t
  in
  List.map clear_goal
...
let open_new_goal ~lemma build_proof sigma using_lemmas ref_ goal_name
  ...
  let hook _ =
    let opacity =
      ...
      | GlobRef.ConstRef c ->
        is_opaque_constant c
      | _ ->
        anomaly ~label:"equation_lemma" (Pp.str "not a constant.")
    in
...
let com_eqn uctx nb_arg eq_name functional_ref f_ref terminate_ref
  ...
  let opacity =
    ...
    | GlobRef.ConstRef c ->
      is_opaque_constant c
    | _ ->
      anomaly ~label:"terminate_lemma" (Pp.str "not a constant.")
  in
...
let recursive_definition ~interactive_proof ~is_mes function_name rec_impls
  ...
  let res =
    ...
    | _ ->
      failwith "Recursive Definition (res not eq)"
  in
