...
let is_rec names =
  ...
  let rec lookup names gt =
    ...
    | GCast (b, _) ->
      lookup names b
    | GRec _ ->
      CErrors.user_err (Pp.str "GRec not handled")
    | GIf (b, _, lhs, rhs) ->
    ...
    | GLetTuple (nal, _, t, b) ->
      ...
      || lookup
           ...
           b
    | GApp (f, args) ->
      List.exists (lookup names) (f :: args)
    | GCases (_, _, el, brl) ->
...
let rec local_binders_length = function
  ...
  | [] ->
    0
  | Constrexpr.CLocalDef _ :: bl ->
    1 + local_binders_length bl
...
let build_functional_principle ?(opaque = Proof_global.Transparent)
    (evd : Evd.evar_map ref) interactive_proof old_princ_type sorts funs i
    proof_tac hook =
  ...
  let {name; entries} =
    Lemmas.pf_fold
      (close_proof ~opaque ~keep_body_ucst_separate:false (fun x -> x))
      lemma
  in
  match entries with
  | [entry] ->
...
let generate_functional_principle (evd : Evd.evar_map ref) interactive_proof
  ...
  try
    ...
    let new_sorts =
      match sorts with
      | None ->
        Array.make (Array.length funs) type_sort
      | Some a ->
        a
    in
    ...
    in
    let entry, hook =
      build_functional_principle evd interactive_proof old_princ_type new_sorts
        funs i proof_tac hook
    in
    (* Pr  1278 :
  ...
  *)
    ...
    let (_ : Names.GlobRef.t) =
      DeclareDef.declare_definition ~name:new_princ_name ~hook_data
      ...
      let _ =
        List.map_i
          (fun i x ->
            ...
            generate_functional_principle evd interactive_proof princ_type None
              None
    ...
    let body =
      ...
      | None ->
        CErrors.user_err ~hdr:"Function"
          Pp.(str "Body of Function must be given")
    in
    ComDefinition.do_definition ~program_mode:false ~name:fname.CAst.v
      ~poly:false ~scope:(DeclareDef.Global Declare.ImportDefaultBehavior)
...
let generate_correction_proof_wf f_ref tcc_lemma_ref is_mes functional_ref
    eq_ref rec_arg_num rec_arg_type nb_args relation (_ : int)
...
let generate_type evd g_to_f f graph i =
  ...
  let fun_ctxt, res_type =
    match ctxt with
    | [] | [_] ->
      CErrors.anomaly (Pp.str "Not a valid context.")
    | decl :: fun_ctxt ->
      (fun_ctxt, RelDecl.get_type decl)
  in
...
let find_induction_principle evd f =
  let f_as_constant, u =
    ...
    | Constr.Const c' ->
      c'
    | _ ->
      CErrors.user_err Pp.(str "Must be used with a function")
  in
    ...
    let prove_branche i g =
      ...
      let pre_args =
        List.fold_right
          (fun {CAst.v = pat} acc ->
            match pat with
            | Tactypes.IntroNaming (Namegen.IntroIdentifier id) ->
              id :: acc
            | _ ->
              CErrors.anomaly (Pp.str "Not an identifier."))
         ...
         [ observe_tac "h_intro_patterns "
             (let l = List.nth intro_pats (pred i) in
              ...
              | [] ->
                tclIDTAC
              | _ ->
                Proofview.V82.of_tactic (intro_patterns false l))
    ...
    let lemmas =
      Array.map
        (fun (_, (ctxt, concl)) ->
          match ctxt with
          | [] | [_] | [_; _] ->
            CErrors.anomaly (Pp.str "bad context.")
          | hres :: res :: decl :: ctxt ->
            let res =
...
let make_scheme evd (fas : (Constr.pconstant * Sorts.family) list) :
  ...
  let first_fun_kn =
    ...
    | None ->
      raise No_graph_found
    | Some finfos ->
      fst finfos.graph_ind
  in
  ...
  let l_schemes =
    List.map
      ( EConstr.of_constr
      %> Typing.unsafe_type_of env sigma
  ...
  let opaque =
    ...
    | None ->
      Transparent (* non recursive definition *)
    | Some equation ->
  ...
  let entry, _hook =
    try
      build_functional_principle ~opaque evd false first_type
        (Array.of_list sorts) this_block_funs 0
  ...
  if List.is_empty other_princ_types then [entry]
    ...
    let first_princ_body, first_princ_type =
      Declare.(entry.proof_entry_body, entry.proof_entry_type)
    in
    let ctxt, fix =
...
let register_wf interactive_proof ?(is_mes = false) fname rec_impls wf_rel_expr
  ...
  let hook ((f_ref, _) as fconst) tcc_lemma_ref (functional_ref, _) (eq_ref, _)
      rec_arg_num rec_arg_type nb_args relation =
    try
      pre_hook [fconst]
...
let register_mes interactive_proof fname rec_impls wf_mes_expr wf_rel_expr_opt
  ...
  let wf_arg_type, wf_arg =
    ...
    | None -> (
      match args with
      | [Constrexpr.CLocalAssum ([{CAst.v = Name x}], k, t)] ->
        (t, x)
      | _ ->
        CErrors.user_err (Pp.str "Recursive argument must be specified") )
    | Some wf_args -> (
      try
        match
          List.find
  ...
  let lemma, _is_struct =
    ...
    | [ ( { Vernacexpr.rec_order =
      ...
      let pre_hook pconstants =
        generate_principle
          ...
          pconstants on_error true register_built fixpoint_exprl recdefs true
      in
    ...
    | _ ->
      ...
      generate_principle (ref !evd) pconstants on_error false register_built
        fixpoint_exprl recdefs interactive_proof
...
let warning_error names e =
  let e_explain e =
    ...
    | _ ->
      if do_observe () then Pp.(spc () ++ CErrors.print e) else Pp.mt ()
  in
  match e with
  | Building_graph e ->
    let names =
          ...
          let rec aux (n : int) = function
            ...
            | CLocalAssum (nal, k, t'') :: nal_ta' ->
              ...
              if n >= nal_l then aux (n - nal_l) nal_ta'
                ...
                raise (Stop new_t')
            | _ ->
              CErrors.anomaly (Pp.str "Not enough products.")
          in
          aux n nal_ta'
        in
...
let rec add_args id new_args =
  ...
  CAst.map (function
    ...
    | (CHole _ | CPatVar _ | CEvar _ | CPrim _ | CSort _) as b ->
      b
    | CCast (b1, b2) ->
      CCast
        ( add_args id new_args b1
          ...
          List.map
            (fun (id, recexp, bl, t, b) ->
              let {CAst.loc; v = rec_id} =
                ...
                | {CAst.v = CStructRec id} ->
                  id
                | {CAst.v = CWfRec (id, _)} ->
                  id
                | {CAst.v = CMeasureRec (oid, _, _)} ->
                  Option.get oid
              in
              let new_args =
                List.flatten
...
let build_case_scheme fa =
  ...
  let funs =
    ...
    try
      ...
      | ConstRef c ->
        c
      | IndRef _ | ConstructRef _ | VarRef _ ->
        assert false
    with Not_found ->
  ...
  let scheme_type =
    EConstr.Unsafe.to_constr
      ((Typing.unsafe_type_of env sigma) (EConstr.of_constr scheme))
  in
