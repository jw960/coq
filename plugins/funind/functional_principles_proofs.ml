  ...
  let rec build_proof_aux do_finalize dyn_infos : tactic =
   fun g ->
    ...
    | App (_, _) -> (
      ...
      | Int _ ->
        user_err Pp.(str "integer cannot be applied")
      | Float _ ->
        user_err Pp.(str "float cannot be applied")
      | App _ ->
              ...
              equation_lemma =
                Some
                  ...
                  | GlobRef.ConstRef c ->
                    c
                  | _ ->
                    CErrors.anomaly (Pp.str "Not a constant.") ) }
        | _ ->
          ()
      in
...
let prove_princ_for_struct (evd : Evd.evar_map ref) interactive_proof fun_num
 ...
 fun g ->
  ...
  let fresh_id =
    ...
    fun na ->
      let new_id =
        match na with
        | Name id ->
          fresh_id !avoid (Id.to_string id)
        | Anonymous ->
          fresh_id !avoid "H"
      in
      avoid := new_id :: !avoid;
  ...
  let get_body const =
    ...
    | None ->
      user_err Pp.(str "Cannot define a principle over an axiom ")
  in
          ...
          (fun i (acc_map, acc_info) decl ->
            ...
            let body_with_param, num =
              ...
              | Fix ((_, num), (_, _, bs)) ->
                ...
                , num )
              | _ ->
                user_err Pp.(str "Not a mutual block")
            in
            let info =
              { infos with
...
let backtrack_eqs_until_hrec hrec eqs : tactic =
 fun gls ->
  ...
  in
  let _, hrec_concl =
    decompose_prod (project gls) (pf_unsafe_type_of gls (mkVar hrec))
  in
  ...
  let rec backtrack : tactic =
   fun g ->
    ...
    | App (f', _) when eq_constr (project g) f' f ->
      tclIDTAC g
    | _ ->
      tclTHEN rewrite backtrack g
  in
  backtrack gls
...
let is_valid_hypothesis sigma predicates_name =
  ...
  let rec is_valid_hypothesis typ =
    ...
    | Prod (_, pte, typ') ->
      is_pte pte && is_valid_hypothesis typ'
    | _ ->
      false
  in
  is_valid_hypothesis
...
let prove_principle_for_gen (f_ref, functional_ref, eq_ref) tcc_lemma_ref is_mes
  ...
  let lemma =
    ...
    | Undefined ->
      user_err Pp.(str "No tcc proof !!")
    | Value lemma ->
      EConstr.of_constr lemma
    | Not_needed ->
      EConstr.of_constr
        (UnivGen.constr_of_monomorphic_global @@ Coqlib.lib_ref "core.True.I")
  in
