...
type pte_info =
  { proving_tac : Id.t list -> unit Proofview.tactic
...
let observe_tac s = observe_tac ~header:(Pp.mt()) (fun _ _ -> Pp.str s)

let finish_proof dynamic_infos = observe_tac "finish" assumption

let refine c = Refine.refine ~typecheck:true (fun sigma -> sigma, c)
(* let refine c = Proofview.V82.tactic (Refiner.refiner ~check:true EConstr.Unsafe.(to_constr c)) *)
...
let change_hyp_with_using msg hyp_id t tac gl =
  let prov_id = pf_get_new_id hyp_id gl in
  tclTHENS
    ((* observe_tac msg *) assert_by (Name prov_id) t (tclCOMPLETE tac))
    [ tclTHENLIST
        [ (* observe_tac "change_hyp_with_using thin" *) (clear [hyp_id])
        ; (* observe_tac "change_hyp_with_using rename " *) (rename_hyp [prov_id,hyp_id])
        ]
    ]

let change_hyp_with_using msg hyp_id t = change_hyp_with_using msg hyp_id t %> PG.enter
...
let prove_trivial_eq h_id context (constructor,type_of_term,term) =
  ...
  tclTHENLIST
    ...
    ; PG.enter (fun gl ->
          let context_hyps =
            fst (list_chop ~msg:"prove_trivial_eq : " nb_intros (pf_ids_of_hyps gl))
          in
          let context_hyps' =
...
let rewrite_until_var arg_num eq_ids =
  ...
  *)
  let test_var gl =
    ...
    let _,args = destApp sigma (pf_concl gl) in
              ...
              tclTHENLIST
                ...
                ; PG.enter (fun gl ->
                      ...
                      (tclTHENS
                         ...
                         ; (* observe_tac "prove rec hyp" *) (refine to_refine)
                         ])
                    )
                ]
            in
...
(*                  );  *)
      ...
      tclIDTAC
  in
  try scan_type [] (Typing.unsafe_type_of env sigma (mkVar hyp_id)), [hyp_id]
  with TOREMOVE ->
    clear [hyp_id],[]
            ...
            ; observe_tac "after_introduction"
                (PG.enter (fun gl->
                     ...
                     let new_term_value =
                       ...
                       | _ ->
                         observe (str "cannot compute new term value : "
                                  ++ Tacmach.pr_gls (PG.print gl) ++ fnl () ++ str "last hyp is" ++
                     ...
                     in
                     let new_body = pf_apply Reductionops.nf_betaiota gl (mkApp(fun_body,[| new_term_value |])) in
                     let new_infos =
                       { dyn_infos with
                         info = new_body
...
let build_proof (interactive_proof:bool) (fnames:Constant.t list) ptes_infos dyn_infos =
    ...
    PG.enter (fun gl ->
            ...
            | Prod _ ->
              tclTHEN
                ...
                (PG.enter (fun gl ->
                     ...
                     let new_term = pf_apply Reductionops.nf_betaiota gl (mkApp(dyn_infos.info,[|mkVar id|])) in
                     ...
                     in
                     (*                         observe_tac "Lambda" *) (instantiate_hyps_with_args do_prove new_infos.rec_hyps [id])
        ...
        | App(_,_) ->
          ...
          begin
            ...
            | LetIn _ ->
              let new_infos =
                { dyn_infos with info = Reductionops.nf_betaiotazeta env sigma dyn_infos.info } in
              tclTHENLIST
                [ tclMAP
                    (fun hyp_id ->
                       h_reduce_with_zeta (Locusops.onHyp hyp_id))
  ...
  and build_proof do_finalize dyn_infos =
    ...
    Indfun_common.New.observe_tac ~header:(Pp.mt()) (fun env sigma ->
        str "build_proof with " ++ pr_leconstr_env env sigma dyn_infos.info ) (build_proof_aux do_finalize dyn_infos)
  and build_proof_args env sigma do_finalize dyn_infos (* f_args'  args *)  =
...
let prove_rec_hyp_for_struct fix_info eq_hyps =
  tclTHEN
    ...
    (PG.enter (fun gl ->
         ...
         let rec_hyp_proof = mkApp(mkVar fix_info.name,array_get_start pte_args) in
...
(*   observe (str "f_body_with_params_and_other_fun " ++  pr_lconstr f_body_with_params_and_other_fun); *)
  ...
  let (type_ctxt,type_of_f), evd =
    ...
    in decompose_prod_n_assum evd (nb_params + nb_args) t,evd
  in
  ...
  let prove_replacement =
    tclTHENLIST
      [
        ...
        observe_tac "" (PG.enter (fun gl ->
           let rec_id = fst @@ List.nth (pf_hyps_types gl) 0 in
           tclTHENLIST
...
let do_replace (evd : Evd.evar_map ref) params rec_arg_num rev_args_id f fun_num all_funs =
  ...
  tclTHEN
    ...
    (PG.enter (fun gl ->
         ...
         tclTHEN (Equality.rewriteLR equation_lemma)
...
let prove_princ_for_struct (evd:Evd.evar_map ref) interactive_proof fun_num fnames all_funs _nparams =
  ...
  (* Array.iter (fun c -> Pp.msgnl (Printer.pr_lconstr c)) all_funs; *)
    ...
    let f_ctxt,f_body = decompose_lam (project gl) fbody in
    ...
    let ptes_to_fix,infos =
      match EConstr.kind (project gl) fbody_with_full_params with
        | Fix((idxs,i),(names,typess,bodies)) ->
            let bodies_with_all_params =
              Array.map
                (fun body ->
                   Reductionops.nf_betaiota (pf_env gl) (project gl)
            ...
            let info_array =
              Array.mapi
                (fun i types ->
                   let types = prod_applist (project gl) types (List.rev_map var_of_decl princ_params) in
                   { idx = idxs.(i)  - fix_offset;
                     ...
                     nb_realargs =
                       List.length
                         (fst (decompose_lam (project gl) bodies.(i))) - fix_offset;
            ...
            let pte_to_fix,rev_info =
              List.fold_left_i
                (fun i (acc_map,acc_info) decl ->
                   ...
                   let type_args,_ = decompose_prod (project gl) infos.types in
                   ...
                   let body_with_param,num =
                     ...
                     in
                     match EConstr.kind (project gl) body_with_full_params with
                       | Fix((_,num),(_,_,bs)) ->
                               Reductionops.nf_betaiota (pf_env gl) (project gl)
                                 (
                                   (applist
        ...
        | _, this_fix_info::others_infos ->
            ...
            then
              ...
              then tclIDTAC (* Someone  tries to defined a principle on a fully parametric definition declared as a fixpoint (strange but ....) *)
                ...
                Indfun_common.New.observe_tac ~header:(mt()) (fun _ _ -> str "h_fix " ++ int (this_fix_info.idx +1))
      ...
      PG.enter (fun gl ->
        ...
        try
          ...
          tclTHENLIST
            ...
            ; PG.enter (fun gl -> (* replacement of the function by its body *)
                  let args = nLastDecls gl nb_args in
        ...
        with Not_found ->
          ...
          tclTHENLIST
            ...
            ; PG.enter (fun gl -> (* replacement of the function by its body *)
                 ...
                 in
                 let fname = destConst (project gl) (fst (decompose_app (project gl) (List.hd (List.rev pte_args)))) in
                 tclTHENLIST
...
let prove_with_tcc tcc_lemma_constr eqs =
...
let backtrack_eqs_until_hrec hrec eqs =
  PG.enter (fun gl ->
  let eqs = List.map mkVar eqs in
  let rewrite = tclFIRST (List.map (fun x -> Equality.rewriteRL x) eqs) in
  ...
  let rec backtrack () =
    ...
    | App(f',_) when eq_constr (project gl) f' f ->
      tclIDTAC
...
let new_prove_with_tcc is_mes acc_inv hrec tcc_hyps eqs =
  (tclTHENLIST
...
let prove_principle_for_gen
  ...
  let wf_tac =
    if is_mes
    then (fun b -> Recdef.tclUSER_if_not_mes tclIDTAC b None)
...
(*   observe (str "rec_arg_id := " ++ pr_lconstr (mkVar rec_arg_id)); *)
  let subst_constrs = List.map (RelDecl.get_name %> Nameops.Name.get_id %> mkVar) (pre_rec_arg@princ_info.params) in
  ...
  in
  let revert l = tclTHEN (Tactics.generalize (List.map mkVar l)) (clear l) in
...
(*                 "apply wf_thm"  *)
  ...
  in
  let args_ids = List.map (RelDecl.get_name %> Nameops.Name.get_id) princ_info.args in
  let lemma =
...
(*   in *)
  ...
  tclTHENLIST
    ...
    ; h_intros
        (List.rev_map (RelDecl.get_name %> Nameops.Name.get_id)
...
(*                         str "fix arg num" ++ int (List.length args_ids + 1) ); tclIDTAC g); *)
      (* observe_tac "h_fix " *) (fix fix_id (List.length args_ids + 1))
    ;
(*       (fun g -> observe (Printer.pr_goal (sig_it g) ++ fnl() ++ pr_lconstr_env (pf_env g ) (pf_unsafe_type_of g (mkVar fix_id) )); tclIDTAC g); *)
    ...
    ; (* observe_tac "finish" *) PG.enter (fun gl ->
          let body =
          ...
          let predicates_names =
            List.map (RelDecl.get_name %> Nameops.Name.get_id) princ_info.predicates
          in
          let pte_info =
            { proving_tac =
                (fun eqs ->
         ...
         (* observe_tac "instantiate_hyps_with_args"  *)
           (instantiate_hyps_with_args
              make_proof
              (List.map (RelDecl.get_name %> Nameops.Name.get_id) princ_info.branches)
              (List.rev args_ids)
           )
      )
    ]
