...
let filter_map filter f =
  let rec it = function
    | [] ->
      []
    | e :: l ->
      if filter e then f e :: it l else it l
  in
  it
...
let list_union_eq eq_fun l1 l2 =
  let rec urec = function
    | [] ->
      l2
    | a :: l ->
      if List.exists (eq_fun a) l2 then urec l else a :: urec l
  in
  urec l1
...
let add_Function is_general f =
  ...
  and graph_ind =
    ...
    | GlobRef.IndRef ind ->
      ind
    | _ ->
      CErrors.anomaly (Pp.str "Not an inductive.")
  in
  let finfos =
    { function_constant = f
    ; equation_lemma
...
let do_rewrite_dependent () =
  !functional_induction_rewrite_dependent_proofs = true
...
let well_founded = function
  | () ->
    EConstr.of_constr (coq_constant "well_founded")
...
let well_founded_ltof () =
  EConstr.of_constr @@ UnivGen.constr_of_monomorphic_global
  @@ Coqlib.find_reference "IndFun" ["Coq"; "Arith"; "Wf_nat"]
       "well_founded_ltof"
