...
let rec parse_args (args : string list) vo_acc : _ * string =
  match args with
  | [] -> CErrors.user_err (Pp.str "parse args error: missing argument")
  | ("-Q" | "-R") :: d :: p :: rem ->
    let p = Loadpath.dirpath_of_string p in
    let vo_acc = (d, p) :: vo_acc in
    parse_args rem vo_acc
  | "-I" :: _d :: rem ->
    (* Ignore *)
    parse_args rem vo_acc
  | "-coqlib" :: s :: rem ->
...
let () =
  ...
  try
    let vo_path, in_file = parse_args (List.tl @@ Array.to_list Sys.argv) [] in
    let () = init_load_path vo_path in
