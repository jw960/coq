...
(* We will introduce a [Make] functor and remove NAMETREE below when libobject and interact with the Nametab *)
(* module Make() .. *)

module GlobRef : S with type elt := GlobRef_.t and type path := full_path
module Abbrev : S with type elt := Globnames.abbreviation and type path := full_path

module ModType : S with type elt := ModPath.t and type path := full_path
module Module : S with type elt := ModPath.t and type path := DirPath.t
module Dir : S with type elt := GlobDirRef.t and type path := DirPath.t

module Universe : S with type elt := Univ.UGlobal.t and type path := full_path
...
(* val locate_all : qualid -> GlobRef_.t list *)
...
val global_of_path : full_path -> GlobRef_.t
 ...
 * val path_of_global : GlobRef_.t -> full_path
...
val dirpath_of_global : GlobRef_.t -> DirPath.t
val basename_of_global : GlobRef_.t -> Id.t

(** Printing of global references using names as short as possible.
    @raise Not_found when the reference is not in the global tables. *)
val pr_global_env : Id.Set.t -> GlobRef_.t -> Pp.t
...
val shortest_qualid_of_global : ?loc:Loc.t -> Id.Set.t -> GlobRef_.t -> qualid
