...
type ('t, 'var, 'pat, 'syn_ext) glob_constr_r =
  | GRef    of 'var
  | GEvar   of existential_name * (Id.t * glob_constr) list
  | GPatVar of bool * patvar (** Used for patterns only *)
  | GApp    of 't * 't list

  | GLambda of Name.t * binding_kind * 't * 't

  | GProd   of Name.t * binding_kind * 't * 't

  | GLetIn  of Name.t * 't * 't option * 't

  | GLetTuple of Name.t list * (Name.t * 't option) * 't * 't

  | GIf   of 't * (Name.t * 't option) * 't * 't
  ...
  | GCases  of case_style
  * 't option
  * ('t * Name.t * (inductive * Name.t list) option) list
  * (Id.t list * cases_pattern list * 't) list

  | GCast of 't * 't cast_type
