...
let open_close_scope (local,opening,sc) =
  Lib.add_leaf (None, inScope (local,opening,OpenScopeItem (normalize_scope sc)))
...
let enable_prim_token_interpretation infos =
  Lib.add_leaf (None, inPrimTokenInterp infos)
...
let declare_arguments_scope_gen req r n (scl,cls) =
  Lib.add_leaf (None, inArgumentsScope (req,r,n,scl,cls))
