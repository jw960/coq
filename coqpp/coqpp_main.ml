...
| SymbToken (t, s) ->
  fprintf fmt "(Pcoq.GExtend.s_token (%a))" print_tok (t, s)
| SymbEntry (e, None) ->
  fprintf fmt "(Pcoq.GExtend.s_nterm %s)" e
| SymbEntry (e, Some l) ->
  fprintf fmt "(Pcoq.GExtend.s_nterml %s (%a))" e print_string l
| SymbSelf ->
  fprintf fmt "Pcoq.GExtend.s_self"
| SymbNext ->
  fprintf fmt "Pcoq.GExtend.s_next"
| SymbList0 (s, None) ->
  fprintf fmt "(Pcoq.GExtend.s_list0 %a)" print_symbol s
| SymbList0 (s, Some sep) ->
  fprintf fmt "(Pcoq.GExtend.s_list0sep (%a) (%a))" print_symbol s print_symbol sep
| SymbList1 (s, None) ->
  fprintf fmt "(Pcoq.GExtend.s_list1 (%a))" print_symbol s
| SymbList1 (s, Some sep) ->
  fprintf fmt "(Pcoq.GExtend.s_list1sep (%a) (%a))" print_symbol s print_symbol sep
| SymbOpt s ->
  fprintf fmt "(Pcoq.GExtend.s_opt %a)" print_symbol s
| SymbRules rules ->
  let pr fmt (r, body) =
...
let print_entry fmt e =
  ...
  fprintf fmt "let () =@ @[Pcoq.grammar_extend@ %s@ None@ @[{ Pcoq.pos=%a; data=%a}@]@]@ in@ "
...
let terminal s =
  ...
  let c = Printf.sprintf "Pcoq.GExtend.s_token (%s \"%s\")" p s in
  SymbQuote c
