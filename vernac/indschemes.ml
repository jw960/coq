...
let do_mutual_induction_scheme ?(force_mutual=false) lnamedepindsort =
  ...
  let declare decl fi lrecref =
    ...
    let proof_output = Lazy.from_val ((decl,Univ.ContextSet.empty),Safe_typing.empty_private_constants) in
...
let do_combined_scheme name schemes =
  ...
  let proof_output = Lazy.from_val ((body,Univ.ContextSet.empty),Safe_typing.empty_private_constants) in
