...
let objInductive : (Id.t * inductive_obj) Libobject.Dyn.tag =
  ...
  declare_named_object_full {(default_object "INDUCTIVE") with
    ...
    open_function = Open_filter.simple_open open_inductive;
...
let declare_primitive_projection p c = Lib.add_leaf (None, inPrim (p,c))
...
let declare_mind ?typing_flags mie =
  ...
  let () = Lib.add_leaf (Some id, inInductive (id, { ind_names = names })) in
