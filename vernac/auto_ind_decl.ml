...
let make_bl_scheme mode mind =
  ...
  let ans, ctx = Pfedit.build_by_tactic ~poly:false ~side_eff (Global.env()) ctx bl_goal
...
let make_lb_scheme mode mind =
  ...
  let ans, ctx = Pfedit.build_by_tactic ~poly:false ~side_eff (Global.env()) ctx lb_goal
...
let make_eq_decidability mode mind =
  ...
  let lnonparrec,lnamesparrec =
    context_chop (nparams-nparrec) mib.mind_params_ctxt in
  let side_eff = side_effect_of_mode mode in
  let ans, ctx = Pfedit.build_by_tactic ~poly:false ~side_eff (Global.env()) ctx
