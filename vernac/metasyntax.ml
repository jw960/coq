...
let add_reserved_notation ~local ~infix ({CAst.loc;v=df},mods) =
  ...
  Lib.add_leaf (None, inSyntaxExtension(local,(ntn,synext)))
...
let add_notation_interpretation ~local env (decl_ntn, main_data, notation_symbols, ntn, syntax_rules) =
  ...
  Lib.add_leaf (None, inNotation notation);
...
let add_notation ~local ~infix deprecation env c ({CAst.loc;v=df},modifiers) sc =
  ...
  syntax_rules_iter (fun sy -> Lib.add_leaf (None, inSyntaxExtension (local,(ntn,sy)))) syntax_rules;
  Lib.add_leaf (None, inNotation notation);
...
let declare_scope local scope =
  Lib.add_leaf (None, inScopeCommand(local,scope,ScopeDeclare))

let add_delimiters local scope key =
  Lib.add_leaf (None, inScopeCommand(local,scope,ScopeDelimAdd key))

let remove_delimiters local scope =
  Lib.add_leaf (None, inScopeCommand(local,scope,ScopeDelimRemove))

let add_class_scope local scope cl =
  Lib.add_leaf (None, inScopeCommand(local,scope,ScopeClasses cl))
...
let declare_custom_entry local s =
  if Egramcoq.exists_custom_entry s then
    ...
    Lib.add_leaf (None, inCustomEntry (local,s))
