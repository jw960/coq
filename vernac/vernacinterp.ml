...
let interp_typed_vernac c ~pm ~stack =
  ...
  | VtModifyProof f ->
    Option.map (Vernacstate.LemmaStack.map_top_pstate ~f:(fun pstate -> f ~pstate)) stack, pm
  | VtReadProofOpt f ->
...
let interp_control_flag ~time_header (f : control_flag) ~st
    (fn : st:Vernacstate.t -> Vernacstate.LemmaStack.t option * DeclareObl.State.t) =
  match f with
  | ControlFail ->
...
and vernac_load ~verbosely fname =
  ...
  let rec load_loop ~pm ~stack =
    try
      ...
      let stack, pm =
        v_mod (interp_control ~st:{ st with Vernacstate.lemmas = stack; program = pm })
...
and interp_control ~st ({ CAst.v = cmd } as vernac) =
  ...
  List.fold_right (fun flag fn -> interp_control_flag ~time_header flag fn)
    ...
    (fun ~st ->
       ...
       else Option.map (Vernacstate.LemmaStack.map_top_pstate ~f:Proof_global.update_global_env) pstack, pm)
    ~st
...
let interp_qed_delayed ~proof ~info ~st pe : Vernacstate.LemmaStack.t option * DeclareObl.State.t =
  let stack = st.Vernacstate.lemmas in
  let pm = st.Vernacstate.program in
  let stack = Option.cata (fun stack -> snd @@ Vernacstate.LemmaStack.pop stack) None stack in
  let pm = match pe with
    | Admitted ->
      Lemmas.save_lemma_admitted_delayed ~pm ~proof ~info
    | Proved (_,idopt) ->
      Lemmas.save_lemma_proved_delayed ~proof ~pm ~info ~idopt
  in
  stack, pm
