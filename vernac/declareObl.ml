...
let get_shrink_obligations =
  Goptions.declare_bool_option_and_ref ~depr:true (* remove in 8.8 *)
    ~key:["Shrink"; "Obligations"] ~value:true
...
let declare_mutual_definition pm l =
  ...
  let kns =
    DeclareDef.declare_mutually_recursive ~scope ~opaque ~kind ~udecl ~ntns
  ...
  let pm =
    if pred rem > 0 then
      ...
      if not (Int.Set.is_empty deps) then
        let pm, _progress = auto pm (Some prg.prg_name) deps None in
        pm
      else pm
    else pm
  in
