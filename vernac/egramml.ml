...
| GramTerminal s :: rem ->
  ...
  let tok = Pcoq.GExtend.s_token (CLexer.terminal s) in
...
| TyStop -> Pcoq.GExtend.r_stop
| TyNext (rem, tok, _) -> Pcoq.GExtend.r_next (ty_erase rem) tok
...
let make_rule f prod =
  ...
  Pcoq.GExtend.production (symb, act)
...
let extend_vernac_command_grammar s nt gl =
  ...
  grammar_extend nt None { pos=None; data=[None, None, rules]}
